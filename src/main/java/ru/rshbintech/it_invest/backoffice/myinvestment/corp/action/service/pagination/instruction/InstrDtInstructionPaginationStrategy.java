...
public class InstrDtInstructionPaginationStrategy implements InstructionPaginationStrategy {
    ...
    public InstructionViewService.PaginatedInstructionsResult getPaginatedInstructions(
        ...
        if (request.getStatus() != null) {
            viewInstructions = repository.findByStatusWithInstrDtPagination(
                    request.getCftid(),
                    request.getStatus(),
                    request.getNextInstrDt(),
                    request.getNextInstrNmb(),
                    request.getLimit() + 1
            );
        } else {
            viewInstructions = repository.findWithInstrDtPagination(
                    request.getCftid(),
                    request.getNextInstrDt(),
                    request.getNextInstrNmb(),
                    request.getLimit() + 1
    ...
    private String generateNextId(List<ViewCaInstruction> instructions, int limit) {
        ...
        return lastInstruction.getInstrDt() + "," + lastInstruction.getInstrNmb();
    }
}
