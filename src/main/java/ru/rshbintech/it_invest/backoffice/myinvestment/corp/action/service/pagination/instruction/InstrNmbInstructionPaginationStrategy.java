...
public class InstrNmbInstructionPaginationStrategy implements InstructionPaginationStrategy {
    ...
    public InstructionViewService.PaginatedInstructionsResult getPaginatedInstructions(
        ...
        if (request.getStatus() != null) {
            viewInstructions = repository.findByStatusWithInstrNmbPagination(
                    request.getCftid(),
                    request.getStatus(),
            ...
            viewInstructions = repository.findWithInstrNmbPagination(
                    request.getCftid(),
                    request.getNextInstrNmb(),
                    request.getLimit() + 1
            );
    ...
    private String generateNextId(List<ViewCaInstruction> instructions, int limit) {
        ...
        return lastInstruction.getInstrNmb().toString();
    }
}
