...
| **app.rs-bank-ws-proxy.proxy-all-requests**  | Нужно ли проксировать все запросы, которые поступают на сервис независимо от настроек быстрого ответа. Данная настройка может пригодиться в случае, если алгоритм проксирования по ветке быстрого ответа падает в ошибку и нужно разобраться, что не так. | -                  | false                     |
