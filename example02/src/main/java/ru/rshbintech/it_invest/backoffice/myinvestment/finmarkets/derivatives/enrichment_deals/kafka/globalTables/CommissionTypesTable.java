...
public class CommissionTypesTable implements GlobalTableTopologyBuilder {

    private final KafkaConfig kafkaConfig;

    @Getter
    private GlobalKTable<String, CommissionType> table;

    public static final JsonSerde<CommissionType> VALUE_SERDE = new JsonSerde<>(CommissionType.class);
    private static final String STORAGE_NAME = "dv-deals-enricher-commission-types-store";

    @Override
    public void build(StreamsBuilder builder) {
        String topic = kafkaConfig.getTopic().getCommissionTypes();

        table = builder.globalTable(topic, Consumed.with(Serdes.String(), VALUE_SERDE),
                Materialized.<String, CommissionType, KeyValueStore<Bytes, byte[]>>as(STORAGE_NAME)
