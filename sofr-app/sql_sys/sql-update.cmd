...
echo SQL-update: checking %SOFR_HOME%\properties\sys\liquibase.properties
if not exist %SOFR_HOME%\properties\sys\liquibase.properties (
  echo SQL-update: %SOFR_HOME%\properties\sys\liquibase.properties not found, operations cancelled
  goto 1
)
...
echo SQL-update: using properties %SOFR_HOME%\properties\sys\liquibase.properties
echo SQL-update: using changelog %changelog%
echo SQL-update: using changelog sys
echo SQL-update: now calling liquibase

call liquibase --defaultsFile=%SOFR_HOME%\properties\sys\liquibase.properties --liquibase-schema-name=%schema% --changelog-file=%changelog% update 
