 ...
 */
    FUNCTION TXGetVirtCountByNum( in_Number IN VARCHAR2 ) RETURN NUMBER;
/**
 ...
 */
    PROCEDURE TXCreateLots( v_BegDate_in IN DATE, v_EndDate_in IN DATE, v_TaxGroup_in NUMBER, v_FIID_in NUMBER, v_IsDebug_in NUMBER DEFAULT 0, v_IsOptim_in NUMBER DEFAULT 0, v_IsRecalc_in NUMBER DEFAULT 0 );
/**
...
IS
  ...
  begin
        ...
        --6g
           ...
           if( v_SecureKind <> Rsb_Secur.AVOIRKIND_ORDINARY_BOND ) then
              ...
              if( Rsb_Secur.IsBuy(Deal.oGrp)=1 or Rsb_Secur.IsAvrWrtIn(Deal.oGrp)=1 OR Is600Reg <> 0) then
                 ...
                 if( (Deal.FI.t_DrawingDate-DateBuy) <> 0 ) then
                    MarketPrice.QuoteValue := PriceBuy + (100 - PriceBuy)*((Deal.DD-DateBuy)/(Deal.FI.t_DrawingDate-DateBuy));
                 else
    ...
    begin
      ...
      MethodDetermEC := RSI_MethodDetermEC(Deal.Tick.t_DealID);

      -- 3.1. Признак обращаемости
      if( ( Rsb_Secur.IsRet_Issue(Deal.oGrp) = 1 )or ( Deal.IsTerm = False ) ) then
        -- 1. Если для сделки задана категория "Способ определения РЦ"
        if( MethodDetermEC != chr(1) ) then
           if( substr(MethodDetermEC, 1, 1) = '1' ) then
              MarketPrice_NEW.IfMarket_New := chr(88);
           else
              MarketPrice_NEW.IfMarket_New := chr(0);
           end if;
        else -- 2. Если категория не задана, то осуществляется поиск курсов на бирже
           if( (RSI_GetDealCountry(Deal, Country) <> CHR(1)) and
               (SPGetRate(Deal.FI.t_FIID, -1, RateTypes.MinRate, Deal.DZ, Deal.DZ-add_months(Deal.DZ,-3), C0, Country, 0, 0, v_IsShare) = 0)
             ) then
              MarketPrice_NEW.IfMarket_New := chr(88);
           else
              MarketPrice_NEW.IfMarket_New := chr(0);
      ...
      if( (MethodDetermEC != chr(1)) and (substr(MethodDetermEC, 1, 1) in('1', '3')) ) then -- 1.1. Если для сделки задана категория "Способ определения РЦ" И первый символ классификатора = "1" или "3"
         K_New := 1;
         -- 1.2. Если категория "Способ определения РЦ" не задана И QuoteValue_New определяется по курсу вида "минимальная цена" и "максимальная цена
         -- Коэффициент переопределим в соответствующем пункте алгоритма
         -- 1.3. Если категория <Способ определения РЦ> не задана И QuoteValue_New определяется как фактическая цена сделки.
         -- Коэффициент переопределим в соответствующем пункте алгоритма
      else -- 2. В остальных случаях К_New = 0.8 для сделок продажи, К_New = 1.2 для сделок покупки
         if( (Rsb_Secur.IsBuy(Deal.oGrp) = 1) or (Rsb_Secur.IsAvrWrtIn(Deal.oGrp) = 1) ) then
            K_New := 1.2;
         else
            K_New := 0.8;
      ...
      if( Deal.IsTerm = True ) then
         ...
         if( DealFISS = 1 ) then
            ...
            MarketPrice_NEW.QuoteValue_New := 0;
            MarketPrice_NEW.Market_New     := 'Факт';
            MarketPrice_NEW.DateMarket_New := Deal.DZ;
        ...
        if( Rsb_Secur.IsRet_Issue(Deal.oGrp) = 1 ) then
          MarketPrice_NEW.MarketPrice_NEW := 0;
          MarketPrice_NEW.QuoteValue_New  := 0;
          ...
          -- I. Если сделка заключена до 01.01.2010
          if( Deal.DZ < TO_DATE('01.01.2010', 'DD.MM.YYYY') ) then
             MarketPrice_NEW.QuoteValue_New := 0;
             MarketPrice_NEW.Market_New     := 'Факт';
             MarketPrice_NEW.DateMarket_New := Deal.DZ;
             MarketPrice_NEW.Val_Market_New := chr(1);
          elsif( MethodDetermEC != chr(1) ) then -- II. Если для сделки задана категория "Способ определения РЦ"
             if( substr(MethodDetermEC, 1, 2) in('10', '20') ) then -- 1. Если первые два символа значения категории сделки "Способ определения РЦ" принимают значение 10 или  20
                MarketPrice_NEW.QuoteValue_New := 0;
                if( MethodDetermEC = '10-00' ) then
                   MarketPrice_NEW.Market_New := 'Анонимные торги';
                elsif( MethodDetermEC in ('10-80', '20-80') ) then
                   if( Rsb_Secur.IsSale(Deal.oGrp) = 1 ) then
                      MarketPrice_NEW.Market_New := 'Погашение';
                   else
                      MarketPrice_NEW.Market_New := 'Размещение';
                   end if;
                end if;
                MarketPrice_NEW.DateMarket_New := Deal.DZ;
                MarketPrice_NEW.Val_Market_New := chr(1);
             elsif( substr(MethodDetermEC, 1, 2) = '11' ) then -- 2. Если первые два символа категории сделки "Способ определения РЦ" принимают значение "11"
                if( (Rsb_Secur.IsBuy(Deal.oGrp) = 1) or (Rsb_Secur.IsAvrWrtIn(Deal.oGrp) = 1) ) then
                   v_Rate     := RateTypes.MaxRate;
                   v_IsMinMax := 2; --максимальная
                else
             ...
             elsif( substr(MethodDetermEC, 1, 2) in('21','31') ) then -- 3. Если первые два символа классификатора "Способ определения РЦ" принимают значения "21" или "31"
                if( v_AvrRoot = RSI_RSB_FIInstr.AVOIRKIND_BOND ) then
                   if( SPGetRate(Deal.FI.t_FIID, -1, RateTypes.CloseRateBl, Deal.DZ, 0, C0, CHR(1), 0, 0, v_IsShare) = 0 ) then
                      MarketPrice_NEW.QuoteValue_New := RSI_GetValByRate(Deal.FI.t_FIID, C0, Deal.DZ);
                      MarketPrice_NEW.Market_New     := 'Информационное агентство';
                      MarketPrice_NEW.DateMarket_New := C0.t_SinceDate;
                      MarketPrice_NEW.Val_Market_New := RSI_GetValMarketByRate(Deal.FI.t_FIID, C0);
                   else
                      MarketPrice_NEW.QuoteValue_New := Deal.Price;
                ...
                elsif( v_AvrRoot in(RSI_RSB_FIInstr.AVOIRKIND_SHARE, RSI_RSB_FIInstr.AVOIRKIND_INVESTMENT_SHARE) ) then
                      if( SPGetRate(Deal.FI.t_FIID, -1, RateTypes.CloseRate, Deal.DZ, Deal.DZ-add_months(Deal.DZ,-1), C0, CHR(1), 0, 0, v_IsShare, -1, 1) = 0 ) then
                      MarketPrice_NEW.QuoteValue_New := RSI_GetValByRate(Deal.FI.t_FIID, C0, Deal.DZ);
                      begin
                        select t_ShortName into MarketPrice_NEW.Market_New from dparty_dbt where t_PartyID = C0.t_Market_Place;
                      exception
             ...
             elsif( substr(MethodDetermEC, 2, 1) = '2' ) then -- 4. Если второй символ классификатора на сделке имеет значение "2"
                NoteValue := GetNoteText(Rsb_Secur.OBJTYPE_SECDEAL, LPAD(Deal.Tick.t_DealID, 34, '0'), 23);
                if( NoteValue != 0 ) then
                   MarketPrice_NEW.QuoteValue_New := NoteValue;
                   MarketPrice_NEW.Market_New     := 'Расчет';
             ...
             else -- 5. Для всех иных значений классификатора
                MarketPrice_NEW.QuoteValue_New := Deal.Price;
                MarketPrice_NEW.Market_New     := 'Факт';
                MarketPrice_NEW.DateMarket_New := Deal.DZ;
                MarketPrice_NEW.Val_Market_New := Deal.CFI;
          ...
          else -- III. При отсутствии категории "Способ определения РЦ" на сделке
             v_Find := 0;
             ...
             if( v_Find = 0 ) then
                ...
                -- 1. Ищем курс минимальная/максимальная цена
                if( (RSI_GetDealCountry(Deal, Country) <> CHR(1)) and
                    (SPGetRate(Deal.FI.t_FIID, -1, v_Rate, Deal.DZ, Deal.DZ-add_months(Deal.DZ,-3), C0, Country, 0, v_IsMinMax, v_IsShare) = 0)
                  ) then
                   v_Find := 1;
             ...
             -- 2. Если по предыдущему пункту ничего не найдено
             if( v_Find = 0 ) then
                if( v_AvrRoot = RSI_RSB_FIInstr.AVOIRKIND_BOND ) then
                   if( SPGetRate(Deal.FI.t_FIID, -1, RateTypes.CloseRateBl, Deal.DZ, 0, C0, CHR(1), 0, 0, v_IsShare) = 0 ) then
                      v_Find := 1;
             ...
             -- 3. Если по предыдущему пункту ничего не найдено
             if( v_Find = 0 ) then
                if( SPGetRate(Deal.FI.t_FIID, -1, RateTypes.TaxRate, Deal.DZ, 0, C0, CHR(1), 0, 0, v_IsShare) = 0 ) then
                   v_Find := 1;
                   MarketPrice_NEW.QuoteValue_New := RSI_GetValByRate(Deal.FI.t_FIID, C0, Deal.DZ);
             ...
             -- 4. Если по предыдущему пункту ничего не найдено
             if( v_Find = 0 ) then
                MarketPrice_NEW.QuoteValue_New := Deal.Price;
                MarketPrice_NEW.Market_New     := 'Факт';
                MarketPrice_NEW.DateMarket_New := Deal.DZ;
      ...
      CURSOR CDealSort(v_BegDealSortCode IN VARCHAR2, v_DealSortPrev IN NUMBER ) IS
                          SELECT *
                           ...
                           WHERE t_FIID          = FIID
                             ...
                             AND (t_DealSort     > v_DealSortPrev OR t_DealSort = 0)
        ...
        BEGIN
          SELECT MIN(t_DealSortCode)
           ...
           WHERE t_FIID     = FIID
             AND t_DealDate = DealDate
             AND t_DealTime = DealTime
             AND t_DealSort = 0; --минимальную непронумерованную
        EXCEPTION
          WHEN NO_DATA_FOUND
            THEN v_BegDealSortCode := CHR(1);
        END;
  ...
  ----Выполняет списание лота продажи SALELOT
    ...
    BEGIN
      ...
      IF ReestrValue.V14 = RSB_SCTXC.TXREG_V14_YES THEN
        ...
        IF (v_PriorPort <> v_SaleLot.t_Portfolio AND v_PriorPort > 0)
        THEN
               ...
               SELECT t_ID, t_Type, FreeAmount
                 ...
                 FROM ( SELECT /*+ INDEX( Buy DSCTXLOT_DBT_IDXE)*/
                         ...
                         WHERE Buy.t_FIID = v_SaleLot.t_FIID
                           ...
                           AND Buy.t_Portfolio = (CASE WHEN Portfolio > -1 THEN Portfolio ELSE Buy.t_Portfolio END)
            ...
            SELECT t_ID, t_Type, t_FreeAmount - LnkAmount
              ...
              FROM ( SELECT Buy.t_ID, Buy.t_Type, Buy.t_FreeAmount,
                      ...
                      WHERE Buy.t_FIID = v_SaleLot.t_FIID
                        ...
                        AND Lot.t_Portfolio = (CASE WHEN Portfolio > -1 THEN Portfolio ELSE Lot.t_Portfolio END)
  ...
  ----Выполняет списание лота прямого репо/размещения займа
    ...
    BEGIN
      ...
      IF ReestrValue.V14 = RSB_SCTXC.TXREG_V14_YES THEN
          ...
          IF(RSB_SECUR.GetObjAttrNumber(v_BofficeKind, 46, RSB_SECUR.GETMAINOBJATTRNODATE(v_BofficeKind, LPAD(v_SaleLot.t_DealID, 34, '0'), 46)) <> chr(1))
          THEN
              v_PriorPort := to_number(RSB_SECUR.GetObjAttrNumber(v_BofficeKind, 46, RSB_SECUR.GETMAINOBJATTRNODATE(v_BofficeKind, LPAD(v_SaleLot.t_DealID, 34, '0'), 46)));
          ELSE
              v_PriorPort := 0;
          END IF;
      ELSE
      ...
      IF ReestrValue.V9 = RSB_SCTXC.TXREG_V9_NO THEN
         ...
         WHILE v_Flag = 1 OR v_Flag = 2 LOOP
            ...
            RSI_TXLinkDirectRepoToBuy(v_SaleLot, v_S, v_SaleLot.t_Portfolio);

            IF (v_S > 0 AND v_SaleLot.T_BUYDATE = v_SaleLot.T_SALEDATE) THEN --однодневки
               RSI_TXLinkOneDayRepoToBuy (v_SaleLot, v_S, v_SaleLot.t_Portfolio);
            END IF;
      ...
      LOOP
        ...
        BEGIN
          ...
          WHILE v_SPS > 0 LOOP
            ...
            SELECT NVL(MAX(TXGetVirtCountByNum(t_DealCodeTS)), 0) + 1
              INTO v_VirtualNum
              FROM dsctxlot_dbt
  ...
  ---- Закрыть налоговый период
    ...
    BEGIN
      ...
      INSERT INTO dsctxlot_dbt (T_DEALID,
                        ...
                        SELECT Tick.t_DealID, --T_DEALID
                         ...
                         WHERE Tick.t_DealID = RQ.t_DocID
                           ...
                           AND RQ.t_FIID = Av.t_FIID
                           AND Av.t_TaxGroup NOT IN (888, 999)
                           AND Tick.t_DealDate/*RSI_NPTO.GetDateFromAvrWrtIn(Tick.t_DealID, RQ.t_FactDate, TO_DATE('01.01.0001','DD.MM.YYYY'))*/ >= v_BegDate_in
                           AND Tick.t_DealDate/*RSI_NPTO.GetDateFromAvrWrtIn(Tick.t_DealID, RQ.t_FactDate, TO_DATE('01.01.0001','DD.MM.YYYY'))*/ <= v_EndDate_in
                           AND RQ.t_State   = RSI_DLRQ.DLRQ_STATE_EXEC
                           AND RQ.t_SubKind = RSI_DLRQ.DLRQ_SUBKIND_AVOIRISS
                           AND RQ.t_Type    = RSI_DLRQ.DLRQ_TYPE_DELIVERY
                           AND Tick.t_ClientID = -1
      ...
      TXPutMsg( 0,
                v_FIID_in,
                TXMES_DEBUG,
                'Лоты по комп. поставкам вставлены' );
    ...
    BEGIN
      ...
      ELSE
        ...
        TXPutMsg( 0,
                  v_FIID_in,
                  TXMES_DEBUG,
                  'TXCreateLots: перед чисткой остатков' );
        TXPutMsg( 0,
                  v_FIID_in,
                  TXMES_OPTIM,
                  'TXCreateLots: перед откатом остатков' );
         ...
         WHERE t_Date >= v_BegDate
           AND ((((v_FIID_in = -1) AND
                  (v_TaxGroup_in = -1 OR v_TaxGroup_in = (select NVL(av.t_TaxGroup, 0)
                                                            from davoiriss_dbt av, dsctxlot_dbt lot
                                                           where lot.t_ID = lnk.t_BuyID
                                                             and av.t_FIID = lot.t_FIID
                                                         )
                  )
        ...
        DELETE
         ...
         WHERE ( ( (lot.t_Type IN ( RSB_SCTXC.TXLOTS_BUY, RSB_SCTXC.TXLOTS_BACKREPO, RSB_SCTXC.TXLOTS_LOANGET )) AND
                   (lot.t_BuyDate >= v_BegDate) ) OR
                 ( (lot.t_Type IN ( RSB_SCTXC.TXLOTS_SALE, RSB_SCTXC.TXLOTS_REPO, RSB_SCTXC.TXLOTS_LOANPUT )) AND
...
--      END IF;
 ...
 FUNCTION TXGetNDSComSum( v_DealID IN NUMBER,
    ...
    BEGIN
      ...
      BEGIN
        IF v_ExcludeDate = TO_DATE('01.01.0001','DD.MM.YYYY')  THEN
          SELECT NVL(Max(t_DrawingDate), TO_DATE('01.01.0001','DD.MM.YYYY')) INTO v_MaxDrawingDate
            FROM dfiwarnts_dbt
           WHERE t_FIID =  v_FIID
             AND t_IsPartial != 'X'
             AND t_DrawingDate BETWEEN v_BegDate AND v_EndDate;
    ...
    BEGIN
      ...
      SELECT NVL(SUM(T_AMOUNT - T_NETTING - T_SALE), 0) INTO v_SumAmount
        FROM DSCTXLOT_DBT
       WHERE T_FIID  = p_GO.T_FIID
         AND T_BUYDATE <= p_GO.T_SALEDATE
         AND T_TYPE = RSB_SCTXC.TXLOTS_BUY
         AND T_ISFREE = CHR(88);
