...
<xsl:stylesheet
    ...
    <!-- Константы с наименованиями для обработки уникальных условий -->
    <xsl:variable name = "unique_cond_01" select = "'ProcessDeals_req'"/>
    <xsl:variable name = "cond_01_var_01" select = "'PartyUID'"/>
    <xsl:variable name = "cond_01_var_02" select = "'DIASOFT'"/>
    <xsl:variable name = "cond_01_var_03" select = "'BIS_ID'"/>
    <xsl:variable name = "cond_01_var_04" select = "'BIS_ID2'"/>
    <xsl:variable name = "cond_01_var_05" select = "'CPTY'"/>
                    ...
                    <xsl:for-each select = "./child::node()">
                        ...
                        <xsl:choose>
                            <xsl:when test = "$xsd/child::node()/xs:element[@name = $teg_name]">
                                <xsl:choose>
                                    <xsl:when test = "$xsd/child::node()/xs:element[(@name = $teg_name) and (@maxOccurs = 'unbounded' or @maxOccurs > 1)]">
                                        <xsl:choose>
                                            <xsl:when test="($serv_name = $unique_cond_01) and ($teg_name = $cond_01_var_01)">
                                                <xsl:if test = "count(../child::node()[local-name() = $teg_name][1]/preceding-sibling::node()) = count(./preceding-sibling::node())">
                                                    <sch:member>
                                                        <sch:name>
                                                        ...
                                                        <sch:value>
                                                            <sch:array>
                                                                <sch:data>
                                                                    <xsl:for-each select = "../child::node()[(local-name() = $teg_name) and (((string(./SystemId/text()) = $cond_01_var_02) or (string(./SystemId/text()) = $cond_01_var_03) or (string(./SystemId/text()) = $cond_01_var_04)) and (string(./SystemNodeId/text()) = $cond_01_var_05))]">
                                                                        <sch:value>
                                                                            <xsl:call-template name = "struct_el_val">
                                                                                <xsl:with-param name = "xsd" select = "$xsd"/>
                                                                                <xsl:with-param name = "teg_name" select = "$teg_name"/>
                                                                            </xsl:call-template>
                                                                        </sch:value>
                                                                    </xsl:for-each>
                                                                </sch:data>
                                                ...
                                                <xsl:if test = "count(../child::node()[local-name() = $teg_name][1]/preceding-sibling::node()) = count(./preceding-sibling::node())">
                                                    <sch:member>
                                                        ...
                                                        <sch:value>
                                                            <sch:array>
                                                                <sch:data>
                                                                    <xsl:for-each select = "../child::node()[local-name() = $teg_name]">
                                                                        <sch:value>
                                                                            <xsl:call-template name = "struct_el_val">
                                                                                <xsl:with-param name = "xsd" select = "$xsd"/>
                                                                                <xsl:with-param name = "teg_name" select = "$teg_name"/>
    ...
    <!-- Шаблон преобразования значения элемента структуры -->
    <xsl:template name = "struct_el_val">
        <xsl:param name = "xsd"/>
        <xsl:param name = "teg_name"/>
        <xsl:choose>
            <xsl:when test = "$xsd/child::node()/xs:element[@name = $teg_name]">
                <xsl:apply-templates select = "self::node()" mode = "body">
                    <xsl:with-param name = "xsd" select = "$xsd/child::node()/xs:element[@name = $teg_name]"/>
