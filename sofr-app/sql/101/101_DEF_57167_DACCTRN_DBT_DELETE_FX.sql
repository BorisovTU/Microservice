...
DECLARE
    ...
    OBJECT_STATUS_NOT_TRANSFER  CONSTANT uTableProcessEvent_dbt.T_STATUS%TYPE := -1420;
    ...
    KIND_OPERATION       CONSTANT doproper_dbt.T_KIND_OPERATION%TYPE := 2037; -- пополнение
...
BEGIN
  ...
  --проверим, что операция не является пополнением(операция 2037)
    BEGIN

        SELECT 1 INTO v_Need_Update
                     FROM DOPRDOCS_DBT DOCS, doproper_dbt OPER, dnptxop_dbt NPTXOP 
                     WHERE    DOCS.T_ACCTRNID = v_ObjectId
                           AND OPER.T_ID_OPERATION = DOCS.T_ID_OPERATION 
                           AND OPER.T_KIND_OPERATION = KIND_OPERATION 
...
/* DELETE */
    ...
    IF DELETING AND  (:OLD.T_USERFIELD4 = CHR(1) OR :OLD.T_USERFIELD4 IS NULL)
                       AND v_ObjectId IS NOT NULL AND v_Need_Update = 0
    THEN
        RAISE_APPLICATION_ERROR (-20001,'Невозможно удалить проводку. Дождитесь ответа из Бисквит');
    END IF; 

/*Если пополнение и статус -1420, то апдейтим статус в 5*/
    IF v_Need_Update = 1 AND v_Status = OBJECT_STATUS_NOT_TRANSFER
    THEN
       UPDATE uTableProcessEvent_dbt SET T_STATUS = OBJECT_STATUS_ERROR, T_TIMESTAMP = SYSDATE  WHERE T_RECID = v_RecId;  
    END IF; 
