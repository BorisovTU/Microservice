DECLARE
  -- Процедура добавления параметра RGDVPFI_SECTION в таблицу dgtkoprm_dbt
  PROCEDURE addGTKOPRM
  IS
    v_Name dgtkoprm_dbt.t_name%TYPE := 'RGDVPFI_SECTION';
    v_ObjKind number := 8;
    v_TypeID number := 6;
    v_Code number;
    v_KoprmID number;
    ...
    IF v_NeedToAdd = true THEN
	  ...
	  SELECT nvl(max(t_koprmid), 0) + 1 INTO v_KoprmID FROM dgtkoprm_dbt r1 WHERE r1.t_koprmid < 15000; -- ограничиваем максимальным системным кодом
          INSERT INTO dgtkoprm_dbt ( 
             ...
             v_KoprmID, v_ObjKind, v_Code, v_Name, v_TypeID, 0
          );
          COMMIT;
  ...
  END;
BEGIN
  addGTKOPRM(); 	-- добавление параметра RGDVPFI_SECTION в таблицу dgtkoprm_dbt
  addCommentar(); 	-- добавление поля t_section varchar2(50) в таблицу d_fo07_tmp
END;
/
