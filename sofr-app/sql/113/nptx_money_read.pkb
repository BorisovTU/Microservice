create or replace package body nptx_money_read as
  ...
  g_buf_servkind_stock    constant number(2) := 1;
  g_buf_servkind_forts    constant number(2) := 15;
  ...
  type t_name_list is table of varchar2(100) index by varchar2(100);
  g_rus_name_list t_name_list;
  g_src_name_list t_name_list;
  g_ext_src t_name_list;
  g_src_ext t_name_list;
  g_rus_name_list_opcode t_name_list;
  ...
  function buf_servkind_stock
    return number deterministic is
  begin
    return g_buf_servkind_stock;
  end buf_servkind_stock; 
     
  function buf_servkind_forts
    return number deterministic is
  begin
    return g_buf_servkind_forts;
  ...
  begin
    select r.req_id
      into l_req_id
      from dnptxop_req_dbt r
     where r.src = p_src
       and r.external_id = p_ext_id
       and r.kind =  case when p_src = 'EFR' then p_kind else r.kind end
  ...
  begin
    if g_rus_name_list.exists(p_src_name)
    then
      return g_rus_name_list(p_src_name);
    end if;
    
    return p_src_name;
  end get_src_name_rus;
  
  function get_src_name_rus_opcode (
    p_src_name varchar2
  ) return varchar2 is
  begin
    if g_rus_name_list_opcode.exists(p_src_name)
    then
      return g_rus_name_list_opcode(p_src_name);
    end if;
    
    return p_src_name;
  end get_src_name_rus_opcode;  
  
  function get_src_name (
    p_src_name varchar2
  ) return varchar2 is
  begin
    if g_src_name_list.exists(p_src_name)
    then
      return g_src_name_list(p_src_name);
    end if;
  ...
  begin
    if g_src_ext.exists(p_src_name)
    then
      return g_src_ext(p_src_name);
    end if;
    
    return null;
  end get_src_ext_name;
  ...
  function get_servkind(
    ...
    v_servkind number;
  begin
    v_servkind := case p_isexchange
                    when 0 then buf_servkind_stock
                    when 1 then buf_servkind_stock
                    when 2 then buf_servkind_currency
                    when 3 then buf_servkind_forts
                    when 4 then buf_servkind_stock
                    else 0
                  end;
    return v_servkind;
  end; 
  ...
  function get_marketid(
    p_kind in dnptxop_req_dbt.kind%type,
    p_isexchange in dnptxop_req_dbt.is_exchange%type
  ) return number is
    v_market_id number;
  begin  
    v_market_id := case 
                    ...
                    when p_kind <> buf_kind_out_otc and p_isexchange = 4 then get_spbex_id()
                    when p_kind <> buf_kind_out_otc and p_isexchange = 1 then get_micex_id()
                    else null
                   end; 
    return v_market_id;
  end;
  ...
  ) return dnptxop_req_dbt%rowtype is
    l_row  dnptxop_req_dbt%rowtype;
  begin
  ...
  begin
    ...
    get_sfcontr_data(p_name          => p_req_row.contract,
                     p_serv_kind     => get_servkind(p_req_row.is_exchange),
                     p_serv_kind_sub => case p_req_row.is_exchange when 0 then 9 else 8 end,
                     p_market_id     => get_marketid(p_req_row.kind, p_req_row.is_exchange),
                     po_sfcontr_id   => l_contract_internal,
                     po_iis          => po_iis);

    get_sfcontr_data(p_name          => p_req_row.contract,
                     p_serv_kind     => get_servkind(p_req_row.is_exchange_target),
                     p_serv_kind_sub => case p_req_row.is_exchange_target when 0 then 9 else 8 end,
                     p_market_id     => get_marketid(p_req_row.kind, p_req_row.is_exchange_target),
                     po_sfcontr_id   => po_contract_tgt,
                     po_iis          => l_iis);

    po_currency    := nptx_money_read.get_fiid_by_ccy(p_ccy => p_req_row.currency);
    ...
    elsif p_req_row.is_exchange = 1
    then
      po_outsum := 0;
    else
      po_outsum := nvl(nptx_money_read.get_rest(p_account       => po_account,
  ...
  function get_func_run_deal_id
    return varchar2 deterministic is
  begin
    return funcobj_utils.get_func_id(p_code => 'run_nptx_money');
  end get_func_run_deal_id;
  ...
  begin
    ...
    return is_allowed_autorun(p_system_name          => l_req_row.src,
                              p_exchange_type        => l_req_row.is_exchange,
                              p_exchange_type_target => l_req_row.is_exchange_target,
  ...
  begin
    ...
    select count(1)
      into l_cnt
      from dfuncobj_dbt f
     where f.t_funcid = get_func_run_deal_id
  ...
  ) return dnptxop_err_dbt.error_name%type is
    l_error_name dnptxop_err_dbt.error_name%type;
  begin
    select e.error_name
      into l_error_name
      from dnptxop_err_dbt e
  ...
  ) return dnptxop_req_dbt.is_exchange%type is 
    v_is_exchange  dnptxop_req_dbt.is_exchange%type;
  begin
    v_is_exchange := case p_scr_system
                      when 'DBO UL' then 
                        case p_trading_platform
                          when 'Внебиржевой рынок' then 0
                          when 'Биржевой рынок' then 1
                          when 'Фондовый рынок' then 1
                          when 'Валютный рынок' then 2
                          when 'Срочный рынок' then 3
                          when 'СПБ' then 4
                        end 
                      else 
                        case p_trading_platform
  ...
  begin
    v_trade_name := case p_is_exchange
                      when 0 then 'Внебиржевой рынок'
                      when 1 then 'Биржевой рынок'
                      when 2 then 'Валютный рынок'
                      when 3 then 'Срочный рынок'
                      when 4 then 'СПБ'
                      else '-'
                    end;
    return v_trade_name;
  ...
  begin
    ...
    select --+ cardinality (kl, 3)
           ...
           r.currency,
           r.amount,
           nptx_money_read.get_trade_name(r.is_exchange) market_place,
           nptx_money_read.get_trade_name(r.is_exchange_target) market_place_tgt,
           r.status_changed
      from dnptxop_req_dbt r
      join dnptxop_status_dbt s on s.status_id = r.status_id
      join dnptxop_err_dbt e on e.error_id = r.error_id
      join dnptxop_kind_dbt k on k.kind_id = r.kind
     ...
     where r.import_time >= p_dbegin
       ...
       and (p_client_code = l_null_varchar or r.client_code = p_client_code)
  ...
  function get_src_SystemOrigin(p_SystemOrigin varchar2) return varchar2 as
  begin
    if g_ext_src.exists(p_SystemOrigin)
    then
      return g_ext_src(p_SystemOrigin);
    else
      return p_SystemOrigin;
    end if;
  end get_src_SystemOrigin;
  
  function get_status_name(p_status_id dnptxop_status_dbt.status_id%type) return dnptxop_status_dbt.status_name%type deterministic as
    v_name dnptxop_status_dbt.status_name%type;
  begin
    select s.status_name into v_name from dnptxop_status_dbt s where s.status_id = p_status_id;
    return v_name;
  exception
    when no_data_found then
      return null;
...
begin
  g_rus_name_list('EFR')     := 'АСОА';
  g_rus_name_list('DBO')     := 'ДБО';
  g_rus_name_list('DBO UL')  := 'ДБО ЮЛ';
  
  g_rus_name_list_opcode('EFR')     := 'АСОА';
  g_rus_name_list_opcode('DBO')     := 'ДБО';
  g_rus_name_list_opcode('DBO UL')  := 'ДБОЮЛ';  
  
  g_src_name_list('АСОА')     := 'EFR';
  g_src_name_list('ДБО')      := 'DBO';
  g_src_name_list('ДБО ЮЛ')   := 'DBO UL';
  
  g_ext_src('ЕФР')     := 'EFR';
  g_ext_src('ДБО ФЛ')  := 'DBO';
  g_ext_src('ДБО ЮЛ')  := 'DBO UL';

  g_src_ext('EFR')     := 'ЕФР';
  g_src_ext('DBO')     := 'ДБО ФЛ';
  g_src_ext('DBO UL')  := 'ДБО ЮЛ';
