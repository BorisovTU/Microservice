DECLARE
   c_objecttype_sec number := 101; /* Списание зачисление ЦБ */
   c_type_string number := 7; 
   c_type_money number := 25;  
   c_type_data number := 9;  
   ...
   END;
BEGIN
   CreateNoteKind(405, 'Признак клиента ВТБ', c_type_string, c_objecttype_sec, chr(0));
   CreateNoteKind(406, 'Срок действия поручения', c_type_data, c_objecttype_sec, chr(0));
   CreateNoteKind(407, 'Дата начала действия поручения', c_type_data, c_objecttype_sec, chr(0));
  COMMIT;
EXCEPTION
   WHEN OTHERS THEN  it_log.log('Ошибка создания примечаний списаний-зачислений ЦБ');
END;
