                 ...
                 NVL (
                    (SELECT MAX (op.t_OperDate)
                      ...
                      WHERE     op.t_DocKind = 4605
                            AND op.t_Client = q.t_ClientID
                            AND op.t_IIS = DECODE (q.IIS, 1, 'X', CHR (0))
                            AND EXTRACT (YEAR FROM op.t_OperDate) = q.RecalcYear),
                    TO_DATE ('01.01.0001', 'DD.MM.YYYY'))
                    AS MaxOpDate,
                 q.IIS
            FROM (SELECT DISTINCT
                         tb.t_ClientID,
                         stb.t_TaxPeriod AS RecalcYear,
                         (CASE WHEN stb.t_TaxBaseKind = 5 THEN 1 ELSE 0 END) AS IIS
                    FROM dnptxtbbuf_dbt tb, dnptxtotalbase_dbt stb
                   WHERE tb.t_Status = 1 AND stb.t_TBID = tb.t_EventID
                     AND stb.t_HoldPITax <> 0
                     AND stb.t_TaxBaseCurrPay = 0
                 ) q
          ORDER BY q.t_ClientID ASC, q.RecalcYear ASC
         )
   LOOP

      IF cClientData.MaxOpDate != TO_DATE ('01.01.0001', 'DD.MM.YYYY') THEN
        ...
        IF cClientData.IIS = 1 THEN
          v_SubKind := 40;                                      --Закрытие ИИС
        END IF;
        ...
        insertOP (v_BegDate,
                  ...
                  (CASE WHEN cClientData.IIS = 1 THEN CNST.SET_CHAR ELSE CNST.UNSET_CHAR END),
                  '55728');
      END IF;
   END LOOP;
