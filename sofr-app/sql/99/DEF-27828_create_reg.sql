...
begin
    PutRegVal('SECUR', 'ПРИОРИТЕЗАЦИЯ ЗАПРОСОВ КОНВЕЙЕР', 'Использовать алгоритм приоритезации запросов конвейера, при котором наиболее тяжелые запросы выполняются в первую очередь', chr(0), 4);
    PutRegVal('РСХБ/БРОКЕРСКОЕ ОБСЛУЖИВАНИЕ/ОТЧЕТ БРОКЕРА', 'РАЗДЕЛЯТЬ_ФОРМИРОВАНИЕ', 'Разделять формирование отчёта на два процесса: формирование xml, и конвертация в выходные форматы', chr(0), 4);
    commit;
end;
