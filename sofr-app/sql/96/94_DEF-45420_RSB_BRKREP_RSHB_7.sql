...
-- /*A09*/   RSI_RSBPARTY.GetPartyCode(q.t_MarketID, 1 /*PTCK_CONTR*/),
  /*A09*/   CASE WHEN q.t_ServKindSub = 9 THEN '' ELSE 
            uGetPatyNameForBrkRep(q.t_MarketID)||'('||(CASE WHEN q.t_ServKind = 1 THEN 'Фондовый рынок' WHEN q.t_ServKind = 21 THEN 'Валютный рынок' WHEN q.t_ServKind = 15 THEN 'Срочный рынок' ELSE CHR(1) END)||')' END, --DAN
  ...
  /*A25*/   RSI_RSBPARTY.GetPartyCode(q.t_PartyID, (case when q.t_Flag1 = CHR(0)
            or q.IsOTC = 1 or q.t_ServKindSub = 9 then 6 /*PTCK_SWIFT*/ else GetPtCodeKindForMarket(q.t_MarketID) end)),
...
--Формирование данных по внутреннему учёту для раздела АКТИВЫ отчета (только фактические данные)
...
BEGIN
          ...
          q1 AS (SELECT s.*
               FROM (SELECT acc.t_Code_Currency as t_FIID,
                            ...
                            -1*rsb_account.restac(acc.t_Account, acc.t_Code_Currency, p_EndDate, acc.t_Chapter, null) as OutRest,
                                (CASE WHEN GetAccPoolID(mcacc.SfContrID, acc.t_Account, acc.t_Code_Currency, acc.t_Chapter) <= 0
                                ...
                                mcacc.t_MarketID, mcacc.t_ServKind, mcacc.t_ServKindSub, mcacc.t_PartyID as ClientID
 ...
 /*MarketID*/    q.t_MarketID
  ...
  WITH q_sf AS (SELECT sf.t_ID, sf.t_PartyID, sf.t_ServKind, sf.t_ServKindSub, mp.t_MarketID
          ...
          q AS (SELECT s.*
               FROM (SELECT inturn.t_FIID as t_FIID,
                            (inturn.t_longposition-inturn.t_shortposition) as InRest,
                            (outturn.t_longposition-outturn.t_shortposition) as OutRest,
                            (inturn.t_longpositioncost-inturn.t_shortpositioncost) as InCostRest,
                       ...
                       FROM ddvfiturn_dbt inturn, ddvfiturn_dbt outturn, q_sf
                      WHERE inturn.t_Client = q_sf.t_PartyID AND outturn.t_Client = q_sf.t_PartyID
                        AND inturn.t_ClientContr = q_sf.t_ID AND outturn.t_ClientContr = q_sf.t_ID
             ...
             (Exists(select 1
                      ...
                      where gr.t_DocKind = rq.t_DocKind
                        ...
                        and (   (rq.t_Type IN (RSI_DLRQ.DLRQ_TYPE_COMISS)                             and tk.t_ClientID = tk.SfPartyID                             and gr.t_TemplNum IN (RSI_DLGR.DLGR_TEMPL_PAYCOM))
                             or (rq.t_Type IN (RSI_DLRQ.DLRQ_TYPE_COMISS)                             and tk.t_IsPartyClient = 'X' and tk.t_PartyID = tk.SfPartyID and gr.t_TemplNum IN (RSI_DLGR.DLGR_TEMPL_PAYCOMCONTR))
             ...
             (Exists(select 1
                      ...
                      where gr.t_DocKind = rq.t_DocKind
                        ...
                        and (   (rq.t_Type IN (RSI_DLRQ.DLRQ_TYPE_COMISS)                             and td.t_ClientID = q.t_PartyID                             and gr.t_TemplNum IN (RSI_DLGR.DLGR_TEMPL_PAYCOM))
                             or (rq.t_Type IN (RSI_DLRQ.DLRQ_TYPE_COMISS)                             and td.t_IsPartyClient = 'X' and td.t_PartyID = q.t_PartyID and gr.t_TemplNum IN (RSI_DLGR.DLGR_TEMPL_PAYCOMCONTR))
                             or (rq.t_Type IN (RSI_DLRQ.DLRQ_TYPE_AVANCE,RSI_DLRQ.DLRQ_TYPE_DEPOSIT)  and rq.t_DealPart = 1                                       and gr.t_TemplNum IN (RSI_DLGR.DLGR_TEMPL_PAYAVANCE))
                             or (rq.t_Type IN (RSI_DLRQ.DLRQ_TYPE_PAYMENT)                            and rq.t_DealPart = 1                                       and gr.t_TemplNum IN (RSI_DLGR.DLGR_TEMPL_PAYMENT))
                             or (rq.t_Type IN (RSI_DLRQ.DLRQ_TYPE_AVANCE)                             and rq.t_DealPart = 2                                       and gr.t_TemplNum IN (RSI_DLGR.DLGR_TEMPL_PAYAVANCE2))
                             or (rq.t_Type IN (RSI_DLRQ.DLRQ_TYPE_PAYMENT,RSI_DLRQ.DLRQ_TYPE_INCREPO) and rq.t_DealPart = 2                                       and gr.t_TemplNum IN (RSI_DLGR.DLGR_TEMPL_PAYMENT2,RSI_DLGR.DLGR_TEMPL_PAYPC))
