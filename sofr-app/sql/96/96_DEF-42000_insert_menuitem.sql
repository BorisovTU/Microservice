...
DECLARE
   ...
   C_CASEITEM            NUMBER := 16598;  --Номер модуля
   C_NAME                VARCHAR2 (255) := 'Форма №0409405_МЗ_2293';
   ...
   BEGIN
      ...
      SELECT NVL (
                (SELECT NVL(MAX(T_INUMBERPOINT), 0)
                   FROM dmenuitem_dbt
                  WHERE     trim(t_sznameitem) = 'Регламентированные'
                        AND T_IPROGITEM = C_IDENTPROGRAM_CODE
                        AND T_ISTEMPLATE = p_istemplate
                        AND T_OBJECTID = p_numberoper),
                -1)
      ...
      SELECT NVL (
                (SELECT NVL(MAX(T_INUMBERPOINT), 0)
                   FROM dmenuitem_dbt
                  WHERE     trim(t_sznameitem) in ('~Ф~ормы отчётов, представляемые в Банк России', 'Формы отчётов, представляемые в Банк России')
                        AND T_IPROGITEM = C_IDENTPROGRAM_CODE
                        AND T_ISTEMPLATE = p_istemplate
                        AND T_OBJECTID = p_numberoper
...
BEGIN
   ...
   FOR i IN (SELECT DISTINCT T_OPER
              ...
              WHERE ROL.T_ROLEID IN (10011, 10012, 10013, 10014, 10026))
   LOOP
