CREATE OR REPLACE PACKAGE BODY RSB_DL724REP_2024
IS
      ...
      IF v_SF_SERVKIND = PTSK_STOCKDL  THEN
        ...
        LOOP
          ...
          IF (one_rec.t_avrkindsroot = RSI_RSB_FIINSTR.AVOIRKIND_DEPOSITORY_RECEIPT) THEN
            BEGIN
              SELECT t_lsin, NVL((TRIM(issuer.t_name) ), CHR(1)) t_IssuerName INTO v_LSIN, v_IssuerName
...
/*
  ...
  exception
    when others then
      ...
      RSB_SPREPFUN.AddRepError('ОШИБКА в RSB_DL724REP_2024.ProcessDO :' ||sqlerrm  );
  ...
  BEGIN
    ...
    IF(pParallelLevel > 0) THEN
      ...
      v_sql_process := 'CALL RSB_DL724REP_2024.ProcessDO('||TO_CHAR(pSessionID)||',:start_id, :end_id, '||
    ...
    ELSE -- для отладки
     ...
     FOR one_do IN (SELECT t_SF_ID FROM D724CONTR_DBT WHERE t_SessionID = pSessionID)
      LOOP
        RSB_DL724REP_2024.ProcessDO(pSessionID,one_do.t_SF_ID,one_do.t_SF_ID,  pBegDate, pEndDate, pIsRep12, pChapter);
      END LOOP;
...
with cl as ( select /*+ materialize */ CL.T_SESSIONID
     ...
     (select NVL(sum(TREST.T_REST_RUB), 0)
       ...
       (select /*+ index(PFI D724PFI_DBT_IDX1)*/ NVL(sum(PFI.T_COSTPFI), 0)
          from D724PFI_DBT PFI
         where PFI.T_SESSIONID = pSessionID
           and PFI.T_PARTYID = CL.T_PARTYID
           and PFI.T_CLIENT_GROUPID = CL.T_CLIENT_GROUPID) +
       ...
       (select /*+ index(PFI D724PFI_DBT_IDX1)*/ NVL(sum(PFI.T_COSTPFI), 0)
         ...
         where PFI.T_SESSIONID = pSessionID
           ...
           and exists (select /*+ index(CONTR D724CONTR_DBT_IDX2 )*/ 1
                  from D724CONTR_DBT CONTR
                 where CONTR.T_SESSIONID = pSessionID
                   and CONTR.T_SF_ID = PFI.T_CONTRID
                   and CONTR.T_IIS = 1)
                  ) +
       ...
       CL_ACCKIND as (
         select CL_SUM.T_SESSIONID
            ...
            ,(case when CL_SUM.ACCAMOUNT < 0 then 1
                   ...
                   when CL_SUM.ACCAMOUNT <= 100000000 then 8
                   when CL_SUM.ACCAMOUNT <= 500000000 then 9
                   when CL_SUM.ACCAMOUNT <= 1000000000 then 10
                   else 11
              end) ACCKIND
              ,(case when CL_SUM.ACCAMOUNTIIS < 0 then 1
                   ...
                   when CL_SUM.ACCAMOUNTIIS <= 100000000 then 8
                   when CL_SUM.ACCAMOUNTIIS <= 500000000 then 9
                   when CL_SUM.ACCAMOUNTIIS <= 1000000000 then 10
                   else 11
              end) ACCKINDIIS
              ,CL_SUM.COUNTCLIENT
  ...
  begin
    for i in (select firest.t_client_groupid,
                     ...
                     nvl(RSB_DL724REP_2024.ConvSum(fin.t_facevalue, fin.t_facevaluefi, 0, p_date), 0) t_facevaluerub,
                     firest.t_PriceRub,
