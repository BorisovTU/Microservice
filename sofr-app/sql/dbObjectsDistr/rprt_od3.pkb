create or replace package body RPRT_OD3 as
  ...
  begin
    ...
    for cur in (select (select t_name from dparty_dbt where t_partyid = (select t_partyid from ddp_dep_dbt where t_code = tick.t_department)) department
                      ...
                      ,leg1.t_cfi cfi
                      ,RPRT_OD3.f_get_dealrepo_account(tick.t_dealid, leg1.t_id, tick.t_dealtype, tick.t_bofficekind, p_dt) deal_account
                      ,RPRT_OD3.f_get_corrinttoeir(tick.t_dealid, fininstr.t_fiid, p_dt) t_corrinttoeir
                      ,RPRT_OD3.f_get_correstreserve(tick.t_dealid, fininstr.t_fiid, p_dt) t_correstreserve
                      ,RPRT_OD3.f_get_estreserve(tick.t_dealid, fininstr.t_fiid, p_dt) t_estreserve
    ...
    loop
      ...
      then
        ...
        v_id_file := it_file.insert_file(p_file_name => 'ОД_3_' || to_char(p_dt, 'yyyymmdd') || '_p' || v_part || '.csv'
