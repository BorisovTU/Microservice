...
DECLARE
  logID VARCHAR2(50) := 'BOSS-194-1 Insert RegParm RegVal';
  v_ParentId NUMBER;
  v_cnt NUMBER := 0;
  v_ID  NUMBER := 0;
...
BEGIN
   ...
   BEGIN
     SELECT t_KeyId INTO v_ParentId FROM DREGPARM_DBT WHERE t_ParentId = v_ParentId AND LOWER(T_NAME) = LOWER('КВАЛИФИКАЦИЯ');
     
   EXCEPTION WHEN NO_DATA_FOUND THEN 
       INSERT INTO DREGPARM_DBT 
          (T_KEYID, T_PARENTID, T_NAME, T_TYPE, T_GLOBAL, T_DESCRIPTION, T_SECURITY, T_ISBRANCH, T_TEMPLATE)
        VALUES 
          (0, v_ParentId, 'КВАЛИФИКАЦИЯ', 0, CHR(0),'Настройки используемые для функционала связанного с квалификацией', CHR(0), CHR(0), CHR(1)) 
        RETURNING T_KEYID INTO v_ParentId;
   END;

   IF (v_ParentId <> 0) THEN
     v_ID := AddRegParm(v_ParentId, 'ДИРЕКТОРИЯ', 2, 'Директория для сохранения Уведомлений по КИ');
     AddRegVal(v_ID, 0, '');
     v_ID := AddRegParm(v_ParentId, 'НАПРАВЛЯТЬ СКРЫТУЮ КОПИЮ', 2, 'Почта брокера для получения писем с уведомлениями');
     AddRegVal(v_ID, 0, 'broker@rshb.ru; UL@rshb.ru');
     v_ID := AddRegParm(v_ParentId, 'ПОДПИСАНТ ПО УМОЛЧАНИЮ', 0, 'ID операциониста для подписания отчетности по КИ');
     AddRegVal(v_ID, 1559, '');
   END IF;
