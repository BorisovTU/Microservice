...
begin
   ...
   if vcnt =1 then
     execute immediate 'drop table DCDRECORDS_DBT cascade constraints';
   end if;
       EXECUTE IMMEDIATE 'CREATE TABLE DCDRECORDS_DBT (
          T_ID                      NUMBER(20)
        , T_GUID                    VARCHAR(40 CHAR)
        , T_REQUESTTIME             TIMESTAMP
        ...
        , T_RECORDPAYMENTID         NUMBER(20)
        , T_RECORDPAYMENTQTYID      NUMBER(20)
...
declare
 cnt number;
begin
  select count(*) into cnt from user_indexes i where upper(i.TABLE_NAME)='DCDRECORDS_DBT' and upper(i.INDEX_NAME)='DCDRECORDS_DBT_IDX2' ;
  if cnt =1 then
    execute immediate 'drop index DCDRECORDS_DBT_IDX2';
  end if;
  execute immediate 'CREATE INDEX DCDRECORDS_DBT_IDX2 ON DCDRECORDS_DBT (T_RECORDPAYMENTQTYID)';
end;
/
