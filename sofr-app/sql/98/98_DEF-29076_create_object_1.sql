DECLARE
  v_ConvTypeID number := 2000; -- конвейер для СО по переоценке
  v_TaskID number := 2000; -- вариант запуска конвейера для СО по переоценке
  v_OperationID number := 2000; -- операция для СО по переоценке
  v_Macro varchar2(32) := 'dvCnvServOp.mac';
  ...
  PROCEDURE addOprType(aOperationID number, aName varchar2, aProc varchar2, aMacro varchar2, aService varchar2)
  IS
  BEGIN
    INSERT INTO dcnvoprtype_dbt ( 
      ...
      aOperationID, aName, aName, aProc, aMacro, chr(1), chr(1)
      , 2001, 2002, aService, 16
    );
    COMMIT;
  EXCEPTION
    WHEN others THEN 
...
BEGIN
  ...
  v_ConvTypeID := addConv(v_ConvTypeID, 'конвейер для СО по переоценке');
  -- 2. Создание общего варианта запуска (или проверка, что нужный вариант запуска есть)
  v_TaskID := addTask(v_TaskID, 'запуск конвейера для СО по переоценке');
  -- 3. Создание варианта запуска
  addTaskCnv(v_TaskID, v_ConvTypeID, 'SelectDealsForServOp', v_Macro);
  -- 4. операция для СО по переоценке
  addOprType(v_OperationID, 'операция для СО по переоценке', 'ExecDeals', v_Macro, v_Service);
