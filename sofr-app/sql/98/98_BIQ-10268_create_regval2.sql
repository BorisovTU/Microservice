...
begin
    PutRegVal('РСХБ/ИНТЕГРАЦИЯ', 'ВЗАИМОДЕЙСТВИЕ С AC CDI', '',0, 0);
    PutRegVal('РСХБ/ИНТЕГРАЦИЯ/ВЗАИМОДЕЙСТВИЕ С AC CDI', 'ИНТЕРВАЛ ПОПЫТОК', 'Интервал времени в секундах, в течение которого ожидается ответ от CDI для событий вида 5053-5056',300, 0);
    PutRegVal('РСХБ/ИНТЕГРАЦИЯ/ВЗАИМОДЕЙСТВИЕ С AC CDI', 'КОЛИЧЕСТВО ПОПЫТОК', 'Rоличество попыток обработки событий вида 5053-5056',3, 0);
    commit;
end;