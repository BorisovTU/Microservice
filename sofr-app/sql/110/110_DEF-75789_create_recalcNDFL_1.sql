/*Формирование операций пересчета по DEF-75789*/
DECLARE
...
BEGIN
   ...
   FOR one_rec IN (SELECT *
                     FROM (SELECT q1.t_Client, y.CurrYear
                             FROM (select column_value as CurrYear from table(sys.odcinumberlist(2021, 2022,2023,2024))) y, 
                                  (SELECT obj.t_Client, MIN (EXTRACT (YEAR FROM obj.t_Date)) as StartYear
   ...
   LOOP 
     ...
     UPDATE DNPTXOBJ_DBT
      ...
      WHERE T_OBJID IN (SELECT obj.t_ObjID
                          FROM dnptxobj_dbt obj
                         WHERE obj.t_Client = one_rec.t_Client
                           AND obj.t_Kind = 370
                           AND obj.t_OutSystCode = 'DEPO'
                           AND obj.t_AnaliticKind2 = 2030
                           AND obj.t_AnaliticKind1 > 0);   
