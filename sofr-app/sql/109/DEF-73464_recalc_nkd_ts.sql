/* Formatted on 08.10.2024 9:39:50 (QP5 v5.149.1003.31008) */
DECLARE
...
BEGIN
   ...
   DELETE FROM DNPTXLNKDIASPAY_DBT
         WHERE T_NKDBTSOBJID IN (SELECT DISTINCT obj.t_objid
                                   FROM DNPTXOBJ_DBT obj
                                  WHERE obj.t_kind = 1115 AND obj.t_cur <> 0);
   ...
   LOOP
      SELECT *
       ...
       WHERE T_PAYMENTID = v_payment_table (indx).T_PAYMENTID AND ROWNUM = 1;
      ...
      it_log.log_handle('RecalcNKDTS','Обновлена запись dnptxnkdreqdias_dbt с id ' || v_reqdiasreq.t_id || ' (paymid: '||v_reqdiasreq.t_paymentid||'). t_Sum(old): ' || v_reqdiasreq.t_Sum || '; t_Sum(new): ' || v_sum);

      update dnptxnkdreqdias_dbt set t_Sum = v_sum where t_id = v_reqdiasreq.t_id;
      ...
      LOOP
         ...
         SELECT T_FACEVALUEFI
           INTO v_Cur
           FROM DFININSTR_DBT
          WHERE T_FIID = v_reqdiasreq.T_FIID;
         ...
         v_txObj.T_SUM :=
            RSI_RSB_FIInstr.ConvSum (one_pay.T_RECEIVEDCOUPONAMOUNT,
                                     v_Cur,
