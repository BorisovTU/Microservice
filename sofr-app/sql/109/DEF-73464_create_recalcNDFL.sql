/*Формирование операций пересчета по DEF-73464*/
DECLARE
   ...
   PROCEDURE CreateOpClient(p_ClientID IN NUMBER, p_StartYear IN NUMBER, p_IIS IN CHAR)
   AS
...
BEGIN
   ...
   FOR one_rec IN (
                     SELECT DISTINCT T_PARTYID AS t_client, dlc.t_IIS
                       FROM DNPTXLNKDIASPAY_DBT dp, ddlcontr_dbt dlc
                      WHERE dlc.t_SfContrID = dp.T_CONTRACTID
                            AND dp.T_NKDBTSOBJID IN (SELECT DISTINCT obj.t_objid
                                                       FROM DNPTXOBJ_DBT obj
                                                      WHERE obj.t_kind = 1115 AND obj.t_cur <> 0)
                 ) 
   LOOP 
     CreateOpClient(one_rec.t_Client, 2024, one_rec.t_iis);  
   END LOOP; 
