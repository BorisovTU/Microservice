...
BEGIN
  ...
  */
  UPDATE DDIASRESTDEPO_DBT r
    ...
    AND rownum = 1
  ;
  x_UpdateTimestamp := SQL%ROWCOUNT;
  /*
    DEF-89774 обновление значений производим не для всех, а только для проблемных бумаг 
  */
  IF( x_Isin IN ('GB00B10RZP78') ) THEN
     UPDATE DDIASRESTDEPO_DBT r
  ...
  IF( x_UpdateTimestamp<>1 and x_UpdateValue<>1 ) THEN
