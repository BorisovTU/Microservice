...
--Создание Шага: Проверка лимитов 
DECLARE
BEGIN
  begin
  ...
  VALUES(40269072,21,1,0,0,CHR(0),0,chr(0),0,0,'wrtavr21PKO.mac',chr(1),chr(1),
  CHR(0),CHR(0),'Проверка лимитов',12700000,CHR(0),CHR(88),CHR(0),0,
  CHR(0),CHR(88),chr(1),0,CHR(0),0,CHR(0),0,CHR(0),CHR(0),chr(88),0);
  COMMIT;
  
EXCEPTION
   WHEN OTHERS THEN it_log.log('Ошибка вставки шага Проверка лимитов');
END;
/

DECLARE
BEGIN
  UPDATE doprostep_dbt SET T_PREVIOUS_STEP = 21 where T_BLOCKID = 40269072 and T_NUMBER_STEP = 22;
  COMMIT;
EXCEPTION
