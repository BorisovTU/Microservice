      ...
      THEN
         ...
         ELSE
           ...
           INSERT INTO dmenuitem_dbt (t_objectid,
            ...
            UPDATE dmenuitem_dbt 
               SET t_sznameitem = ' OLD[ОД_1] Расшифровка долговых ценных бумаг', t_inumberfather = V_NUMBERFATHER, t_inumberline = v_NumberLine
             WHERE t_objectid   = p_numberoper
               AND t_istemplate = p_istemplate
               AND t_icaseitem  = 519
               AND t_iidentprogram = C_IDENTPROGRAM_CODE;
...
BEGIN
   ...
   FOR i IN (SELECT DISTINCT t_objectid, t_istemplate FROM DMENUITEM_DBT WHERE t_icaseitem IN (519, 523) AND t_iidentprogram = C_IDENTPROGRAM_CODE AND t_iprogitem = C_IDENTPROGRAM_CODE AND t_csystemitem = chr(0))
   LOOP
      ...
      THEN
         ...
         ELSE
           ...
           INSERT INTO dmenuitem_dbt (t_objectid,
            ...
            UPDATE dmenuitem_dbt 
               SET t_sznameitem = ' OLD[ОД_3] Сделки РЕПО', t_inumberfather = V_NUMBERFATHER, t_inumberline = v_NumberLine
             WHERE t_objectid   = p_numberoper
               AND t_istemplate = p_istemplate
               AND t_icaseitem  = 552
               AND t_iidentprogram = C_IDENTPROGRAM_CODE;
...
BEGIN
   ...
   INSERT INTO DITEMSYST_DBT(T_CIDENTPROGRAM,T_ICASEITEM,T_IKINDMETHOD,T_IKINDPROGRAM,T_IHELP,T_RESERVE,T_SZNAMEITEM,T_PARM )
      ...
      FOR i IN (SELECT DISTINCT t_objectid, t_istemplate FROM DMENUITEM_DBT WHERE t_icaseitem IN (552) AND t_iidentprogram = C_IDENTPROGRAM_CODE AND t_iprogitem = C_IDENTPROGRAM_CODE AND t_csystemitem = chr(0))
   LOOP
      InsertMenu (i.t_objectid, i.t_istemplate);
   END LOOP;
