/*Формирование операций пересчета по DEF-60930*/
DECLARE
...
BEGIN
   ...
   FOR one_rec IN (SELECT *
                     FROM (SELECT q1.t_Client, y.CurrYear
                             FROM (select column_value as CurrYear from table(sys.odcinumberlist(2022,2023,2024))) y, 
                                  (SELECT q.t_Client, MIN(q.StartYear) as StartYear
                                     FROM ( SELECT DISTINCT lot.t_Client, EXTRACT(YEAR FROM comm.T_COMMDATE) as StartYear
                                              FROM DDL_COMM_DBT comm,
                                                   dnptxgo_dbt gl,
                                                   dnptxlot_dbt lot
                                             WHERE     comm.T_DOCKIND IN (135)               --(Глобальная операция с ц/б)
                                                   AND comm.T_COMMDATE >= TO_DATE ('01.01.2022', 'DD.MM.YYYY')
                                                   AND comm.T_CommStatus = 2                                     --Закрыта
                                                   AND comm.t_OperSubKind = 1                                --Конвертация
                                                   AND gl.t_DocKind = comm.t_DocKInd
                                                   AND gl.t_DocumentID = comm.t_DocumentID
                                                   AND lot.t_GOID = gl.t_ID
                                            UNION
                                            SELECT /*+ leading(lot)  index(lot DNPTXLOT_DBT_IDX9)*/ DISTINCT lot.t_Client, EXTRACT(YEAR FROM lot1.t_BuyDate) as StartYear
                                              FROM dnptxlot_dbt lot, dnptxlot_dbt lot1
                                             WHERE lot.t_CLGOID > 0
                                               AND NOT EXISTS(SELECT 1 FROM dnptxgo_dbt gl where gl.t_ID = lot.t_CLGOID)
                                               AND lot1.t_DocKind = lot.t_DocKind
                                               AND lot1.t_DocID = lot.t_DocID
                                               AND lot1.t_FIID <> lot.t_FIID
                                               AND lot1.t_BuyDate >= TO_DATE('01.01.2022','DD.MM.YYYY')
                                            UNION
                                            SELECT DISTINCT lot.t_Client, EXTRACT(YEAR FROM lot.t_BuyDate) as StartYear
                                              FROM dnptxlot_dbt lot
                                             WHERE lot.t_GOID > 0
                                               AND NOT EXISTS(SELECT 1 FROM dnptxgo_dbt gl where gl.t_ID = lot.t_CLGOID)
                                               AND lot.t_BuyDate >= TO_DATE('01.01.2022','DD.MM.YYYY')
                                          ) q
                                   GROUP BY q.t_Client
                                 ) q1
                           WHERE q1.StartYear <= y.CurrYear
                          )
                   ORDER BY CurrYear ASC, t_Client ASC
                 ) 
   LOOP 
     CreateOpClient(one_rec.t_Client, one_rec.CurrYear);  
   END LOOP; 
