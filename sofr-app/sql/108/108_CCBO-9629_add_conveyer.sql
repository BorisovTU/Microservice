-- Настройка конвейера
DECLARE
  v_ConvTypeID number := 1005; -- конвейер для исполнения сделок Кондор
  v_TaskID number := 1005; -- вариант запуска конвейера для исполнения сделок Кондор
  v_OperationID number := 1005; -- операция для исполнения сделок Кондор
...
BEGIN
  ...
  v_ConvTypeID := addConv(v_ConvTypeID, 'Исполнение сделок Кондор');
  -- 2. Создание общего варианта запуска (или проверка, что нужный вариант запуска есть)
  v_TaskID := addTask(v_TaskID, 'Запуск конвейера по исполнению сделок Кондор');
  -- 3. Создание варианта запуска
  addTaskCnv(v_TaskID, v_ConvTypeID, 'SelectKondorDealsCnv', v_Macro);
  -- 4. операция для СО по переоценке
  addOprType(v_OperationID, 'Пакетная операция исполнения сделок Кондор', 'Исполнение сделок Кондор', 'ExecKondorDealsCnv', v_Macro, v_Service);
