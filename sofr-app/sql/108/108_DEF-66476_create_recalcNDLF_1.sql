/*Формирование операций пересчета по DEF-66476*/
DECLARE
   ...
   BEGIN
     ...
     WHILE v_CurYear <= v_EndYear --По факту тут только одна итерация
     LOOP
   ...
   FOR one_rec IN (SELECT *
                     FROM (SELECT q1.t_Client, y.CurrYear
                             FROM (select column_value as CurrYear from table(sys.odcinumberlist(2022,2023,2024))) y, 
                                  (SELECT /*+ leading(ATC dobjatcor_dbt)*/LS.T_CLIENT, MIN (EXTRACT (YEAR FROM S.T_DATE)) as StartYear
                                     FROM DOBJATCOR_DBT ATC,
                                          DNPTXLNK_DBT S, 
                                          DNPTXLOT_DBT LS 
                                    WHERE  ATC.T_OBJECTTYPE = 101
                                          AND ATC.T_GROUPID = 55
                                          AND ATC.T_ATTRID = 1   
                                          AND LS.T_DOCKIND = 101 
                                          AND LS.T_DOCID = TO_NUMBER(ATC.T_OBJECT)
                                          AND LS.T_DEALDATE BETWEEN TO_DATE('01.01.2022','DD.MM.YYYY')  AND TO_DATE('31.12.2024','DD.MM.YYYY')
                                          AND LS.T_KIND = 2
