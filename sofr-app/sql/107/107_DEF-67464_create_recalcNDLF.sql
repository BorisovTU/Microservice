/*Формирование операций пересчета по DEF-67464*/
DECLARE
...
BEGIN
   ...
   FOR one_rec IN (SELECT DISTINCT lot.t_Client
                     FROM DDL_COMM_DBT comm,
                          dfininstr_dbt fin,
                          DSCDLFI_DBT dlfi,
                          dfininstr_dbt newfin,
                          dnptxgo_dbt gl,
                          dnptxlot_dbt lot
   ...
   LOOP 
     CreateOpClient(one_rec.t_Client, 2024);  
   END LOOP; 
