--РСХБ\БРОКЕРСКОЕ ОБСЛУЖИВАНИЕ\ОТЧЕТ БРОКЕРА\ИСКАТЬ_ПОСРОЧ_КОМИСС_030323
DECLARE
 v_ParentID NUMBER(10);
 v_KeyID    NUMBER(10); 
BEGIN

  SELECT t_KeyID INTO v_ParentID
  FROM dregparm_dbt 
  WHERE t_Name = 'РСХБ' AND t_ParentID = 0;

  SELECT t_KeyID INTO v_ParentID
  FROM dregparm_dbt 
  WHERE t_Name = 'БРОКЕРСКОЕ ОБСЛУЖИВАНИЕ' AND t_ParentID = v_ParentID;
  
  SELECT t_KeyID INTO v_ParentID
  FROM dregparm_dbt 
  WHERE t_Name = 'ОТЧЕТ БРОКЕРА' AND t_ParentID = v_ParentID;

  


    INSERT INTO dregparm_dbt(T_KEYID, T_PARENTID, T_NAME, T_TYPE, T_GLOBAL, T_DESCRIPTION, T_SECURITY, T_ISBRANCH, T_TEMPLATE)
       VALUES(0, v_ParentID, 'ИСКАТЬ_ПОСРОЧ_КОМИСС_030323', 4, 'X', 'Признак поиска в отчет просроченных брокерских комиссий по сделкам, начисленных 03.03.2023 г.', chr(0), chr(0), chr(1))
     RETURNING T_KEYID INTO v_KeyID;

    INSERT INTO dregval_dbt(T_KEYID, T_REGKIND, T_OBJECTID, T_BLOCKUSERVALUE, T_EXPDEP, T_LINTVALUE, T_LDOUBLEVALUE, T_FMTBLOBDATA_XXXX)
       VALUES(v_KeyID, 0, 0, chr(0), 0,88, 0, '');

EXCEPTION
  WHEN OTHERS THEN NULL;
END;
/  