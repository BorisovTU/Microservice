...
DECLARE
   ...
   C_CASEITEM            NUMBER := 20170;  --Номер модуля
   C_NAME                VARCHAR2 (255) := 'Краткая справка по расчету НДФЛ до 2022 г. и ранее';
   V_NUMBERFATHER        NUMBER := 0;
   C_NUMBERLINE          NUMBER := 5;
   ...
   BEGIN
      ...
      SELECT NVL (
                (SELECT NVL(MAX(T_INUMBERPOINT), 0)
                   FROM dmenuitem_dbt
                  WHERE     trim(replace(t_sznameitem, '~', '')) = 'Краткая справка по расчету НДФЛ'
