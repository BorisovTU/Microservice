--Привязка счетов 45818, открытых в банке по соответствующей категории учета к соответствующим субдоговорам
BEGIN
  ...
  SELECT /*+ ordered full(acc)*/
         ...
         1 --t_mcbranch
    FROM daccount_dbt acc, daccount_dbt acc_306, dmcaccdoc_dbt mc, dsfcontr_dbt sf, ddlcontrmp_dbt mp, ddlcontr_dbt dl, dmccateg_dbt cat, dmctempl_dbt templ  
   WHERE acc.t_Balance = '45818'
     ...
     AND (    SUBSTR(acc.t_Account, 12) = SUBSTR(acc_306.t_Account, 12)
           OR (acc.t_Account = '45818810599000559026' AND acc_306.t_Account = '30601810399000087839')
           OR (acc.t_Account = '45818810499000050764' AND acc_306.t_Account = '30601810899000115719')
           OR (acc.t_Account = '45818810599000900002' AND acc_306.t_Account = '30601810799000104587')
         )
     ...
     AND cat.t_Code = 'Треб. с н.с. брок. резерв'
