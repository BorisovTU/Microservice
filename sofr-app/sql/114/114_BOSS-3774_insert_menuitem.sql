BEGIN
   ...
   INSERT INTO DITEMSYST_DBT(T_CIDENTPROGRAM,T_ICASEITEM,T_IKINDMETHOD,T_IKINDPROGRAM,T_IHELP,T_RESERVE,T_SZNAMEITEM,T_PARM)
               VALUES('S',26100,1,2,0,CHR(1),'Отчет Уведомления клиентам "Базовый стандарт"','');

END;
/

/*Добавление БО ЦБ\ Отчеты\ Отчет Уведомления клиентам "Базовый стандарт" */
DECLARE
   v_cnt                 NUMBER := 0;
   ...
   C_NAME                VARCHAR2(255) := 'Отчет Уведомления клиентам "Базовый стандарт"';
   V_NUMBERFATHER        NUMBER := 0;
   C_NUMBERLINE          NUMBER := 140;
      ...
      SELECT NVL (
                (SELECT NVL(MAX(T_INUMBERPOINT), 0)
                  ...
                  WHERE     trim(t_sznameitem) = 'Отчеты'
                        and trim(t_sznameprompt) = 'Отчеты бэк-офиса'
                        AND T_IPROGITEM = 83
