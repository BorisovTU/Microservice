...
BEGIN
   ...
   FOR cClientData
      IN (SELECT lot.t_Client,
                 NVL((SELECT MIN(lnk.t_Date) 
                        FROM dnptxlot_dbt l, dnptxlnk_dbt lnk 
                       WHERE l.t_DocKind = 127
                         AND l.t_Client = lot.t_Client
                         AND NOT EXISTS(SELECT 1 FROM ddl_tick_dbt tk WHERE tk.t_DealID = l.t_DocID)
                         AND lnk.t_Client = l.t_Client 
                         AND (lnk.t_BuyID = l.t_ID OR lnk.t_SaleID = l.t_ID)
                     ), TO_DATE('01.01.0001','DD.MM.YYYY')) as MinLnkDate,
                 NVL((SELECT MAX(op.t_OperDate) 
                        FROM dnptxop_dbt op
                       WHERE op.t_DocKind = 4605
                         AND op.t_Client = Lot.t_Client
                     ), TO_DATE ('01.01.0001', 'DD.MM.YYYY')) as MaxOpDate
            FROM dnptxlot_dbt lot
           WHERE lot.t_DocKind = 127
             AND NOT EXISTS(SELECT 1 FROM ddl_tick_dbt tk WHERE tk.t_DealID = lot.t_DocID)
           GROUP BY lot.t_Client
           ORDER BY lot.t_Client
         )
   LOOP 
     v_Year := EXTRACT(YEAR FROM cClientData.MaxOpDate);

     IF CClientData.MinLnkDate > TO_DATE('01.01.0001','DD.MM.YYYY') THEN
       v_Year := EXTRACT(YEAR FROM cClientData.MinLnkDate);
     END IF;

     WHILE v_Year <= EXTRACT(YEAR FROM cClientData.MaxOpDate) 
     LOOP
       IF v_Year = v_CurYear THEN
         v_BegDate := TO_DATE ('01.01.'||TO_CHAR(v_Year), 'DD.MM.YYYY');
         v_EndDate := cClientData.MaxOpDate;
