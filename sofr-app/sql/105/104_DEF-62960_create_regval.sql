...
DECLARE
  logID VARCHAR2(50) := 'DEF-62960 Insert RegParm RegVal';
  v_ParentId NUMBER;
  v_cnt NUMBER := 0;
  v_ID  NUMBER := 0;
...
BEGIN
   ...
   IF (v_ParentId <> 0) THEN
     v_ID := AddRegParm(v_ParentId, 'ВКЛЮЧИТЬ_ЛОГИ_GETBROKERCONTRACT', 4, 'Включение/Отключение дополнительного логирования для сервиса GetBrokerContract');
     AddRegVal(v_ID, 0, '');
   END IF;
