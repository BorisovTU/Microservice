-- Создание пункта меню для пользователя 1
DECLARE
   v_cnt                 NUMBER := 0;
   ...
   BEGIN
      ...
      SELECT NVL (
                (SELECT NVL(MAX(T_INUMBERPOINT), 0)
                   FROM dmenuitem_dbt
                  WHERE     trim(t_sznameitem) in ('Отчеты')
                        AND T_IPROGITEM = C_IDENTPROGRAM_CODE
                        AND T_ISTEMPLATE = p_istemplate
                        AND T_OBJECTID = p_numberoper
        ...
        SELECT NVL (
                (SELECT NVL(MAX(T_INUMBERPOINT), 0)
                   FROM dmenuitem_dbt
                  WHERE     trim(t_sznameitem) in ('Поручения и операции клиентов')
                        AND T_IPROGITEM = C_IDENTPROGRAM_CODE
                        AND T_ISTEMPLATE = p_istemplate
                        AND T_OBJECTID = p_numberoper
                        AND t_inumberfather = V_NUMBERFATHER),
     ...
     v_NumberPoint :=  v_NumberPoint + 1;      
         ...
         INSERT INTO dmenuitem_dbt (t_objectid,
              ...
              VALUES (p_numberoper,
                      ...
                      20202,
                      CHR (88),
                      ' ' || ' Суммы подтвержденных расходов',
                      ' ' || ' Отчет "Суммы подтвержденных расходов"',
                      0,
                      83);
