-- Изменения по BOSS-2609_BOSS-2983
-- Добавление настройки 'РСХБ/ДИРЕКТОРИИ/IMPORT_844_DEALS'
DECLARE
  logID VARCHAR2(32) := 'BOSS-2609_BOSS-2983';
  v_ParentId NUMBER;
  v_cnt NUMBER := 0;
  v_ID  NUMBER := 0;
  ...
  PROCEDURE AddNewReg( 
    ...
    , p_KeyDesc IN VARCHAR2
    , p_type IN NUMBER
    , p_intValue IN NUMBER
    , p_blobValue IN VARCHAR2 
  )
  AS
    x_KeyId number;
  BEGIN
    ...
    INSERT INTO dregparm_dbt (
       ...
       0, p_ParentId, p_type, p_KeyName
       , 'X', p_KeyDesc, chr(0), chr(0)
    )
    ...
    INSERT INTO dregval_dbt (
       ...
       x_KeyId, 0, 0, chr(0), 0
       , p_intValue, 0
       , CASE WHEN p_blobValue = '' THEN null ELSE to_blob(utl_raw.cast_to_raw(p_blobValue)) END
    );
  ...
  END;
  -- Изменение настройки
  PROCEDURE UpdateReg( p_KeyID IN number, p_intValue IN NUMBER, p_blobValue IN VARCHAR2 )
  AS
  BEGIN
    LogIt('Изменение настройки '''||p_KeyID||'''');
    UPDATE dregval_dbt 
      SET T_FMTBLOBDATA_XXXX = CASE WHEN p_blobValue = '' THEN null ELSE to_blob(utl_raw.cast_to_raw(p_blobValue)) END
      WHERE T_KEYID = p_KeyId;
    EXECUTE IMMEDIATE 'COMMIT';
  ...
  BEGIN
    ...
    IF(x_Count > 0) THEN
      ...
      UpdateReg( x_KeyId, p_intValue, p_blobValue );
    ELSE
      -- настройки нет
      AddNewReg( x_ParentId, p_KeyName, p_KeyDesc, p_type, p_intValue, p_blobValue );
    END IF;
  EXCEPTION
    WHEN OTHERS THEN
       LogIt('Ошибка добавления настройки '''||p_KeyName||'''');
...
BEGIN
  ...
  AddReg( 
    'РСХБ/ДИРЕКТОРИИ'
    , 'IMPORT_844_DEALS'
    , 'каталог информации о сделках BlockedOTC'
    , 2                                                               -- тип: строка
    , 0
    , '\\sgo-fc01-r03.go.rshbank.ru\sofr_for_etl\inbox\message\' 
  );
