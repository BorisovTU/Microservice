/*Формирование операций пересчета по DEF-62066*/
DECLARE
...
BEGIN
   ...
   FOR one_rec IN (SELECT q.t_Client 
                     FROM (SELECT DISTINCT nptxop.t_Client 
                             FROM dnptxop_dbt nptxop 
                            WHERE nptxop.t_DocKind = 4605 
                                  AND nptxop.t_OperDate BETWEEN TO_DATE ('01.01.2023', 'DD.MM.YYYY') 
                                                            AND TO_DATE ('30.12.2023', 'DD.MM.YYYY') 
                                  AND nptxop.t_SubKind_Operation IN (20) 
                                  ...
                                  AND NOT EXISTS 
                                             (SELECT 1 
                                               ...
                                               WHERE objatcor.t_ObjectType = 132 
                                                     AND objatcor.t_Object = LPAD (nptxop.t_ID, 34, '0') 
                                                     AND objatcor.t_GroupID = 1 
                                                     AND objatcor.t_AttrID = 1) 
                                  AND NOT EXISTS 
                                         (SELECT 1 
                                            FROM dnptxop_dbt wrt 
                                           WHERE     wrt.t_DocKind = 4607 
                                                 AND wrt.t_Client = nptxop.t_Client 
                                                 AND wrt.t_Subkind_Operation = 20 
                                                 AND wrt.t_OperDate = nptxop.t_OperDate 
                                                 AND TRIM (NVL (rsb_struct. 
                                                                 getString (rsi_rsb_kernel. 
                                                                             GetNote ( 
                                                                               131, 
                                                                               LPAD (wrt.t_ID, 
                                                                                     34, 
                                                                                     '0'), 
                                                                               103 /*Отметка об отказе в исполнении*/ 
                                                                                  , 
                                                                               wrt.t_OperDate)), 
                                                                CHR (1))) = CHR (1))) q 
                    WHERE EXISTS (SELECT 1 
                                    FROM dnptxobj_dbt obj 
                                   WHERE obj.t_Client = q.t_Client AND obj.t_Kind IN (select k.t_Element from dnptxkind_dbt k where k.t_Level = 5) 
                                         AND obj.t_Date BETWEEN TO_DATE ('01.01.2023', 'DD.MM.YYYY') 
                                                            AND TO_DATE ('30.12.2023', 'DD.MM.YYYY'))
 
                 ORDER BY q.t_Client ASC
                 ) 
   LOOP 
     CreateOpClient(one_rec.t_Client, 2023);  
   END LOOP; 
