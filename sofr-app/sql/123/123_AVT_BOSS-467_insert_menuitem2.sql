DECLARE
   ...
   C_CASEITEM            NUMBER := 20261; --Номер модуля
   C_NAME                VARCHAR2(255) := 'Контроль задолженности (динамический шаблон)';
   ...
   BEGIN
      ...
      SELECT NVL (
                (SELECT NVL(MAX(T_INUMBERPOINT), 0)
                   FROM dmenuitem_dbt
                  WHERE     replace(trim(t_sznameitem), '~', '') = 'Контроль задолженности по НДФЛ'
      ...
      THEN
         ...
         INSERT INTO dmenuitem_dbt (t_objectid,
              ...
              VALUES (p_numberoper,
                      ...
                      0,
                      CHR (88),
                      'Контроль задолженности по НДФЛ' ,
                      'Контроль задолженности по НДФЛ' ,
                      0,
                      C_IDENTPROGRAM_CODE);
      END IF;
  ...
  IS
   BEGIN
      DELETE FROM  dmenuitem_dbt 
        where t_sznameitem like ' Контроль задолженности по НДФЛ' 
           and t_objectid = p_numberoper
           and t_istemplate = p_istemplate
           and t_iidentprogram = C_IDENTPROGRAM_CODE;
   END;
  ...
  IS
   BEGIN
      ...
      UPDATE dmenuitem_dbt SET t_inumberline = 0, t_inumberfather = V_NUMBERFATHER, t_sznameitem = 'Контроль задолженности по НДФЛ (2024 год)'
        where t_sznameitem = ' Контроль задолженности по НДФЛ (2025 год)' 
           and t_objectid = p_numberoper
           and t_istemplate = p_istemplate
           and t_iidentprogram = C_IDENTPROGRAM_CODE;
   END;
...
BEGIN
   INSERT INTO DITEMSYST_DBT(T_CIDENTPROGRAM,T_ICASEITEM,T_IKINDMETHOD,T_IKINDPROGRAM,T_IHELP,T_RESERVE,T_SZNAMEITEM,T_PARM)
               VALUES('S',C_CASEITEM,1,2,0,CHR(1),C_NAME,'');
