CREATE OR REPLACE TRIGGER "TR_DSFCONTRPLAN_DBT_CDC"
  FOR update or delete or insert
   OF t_sfplanid,t_begin
   ON DSFCONTRPLAN_DBT
  compound trigger
  after each row is
...
BEGIN
   ...
   IF :NEW.T_ID IS NOT NULL THEN
      IF :OLD.T_ID IS NOT NULL THEN
         msgmeta := '<XML action = "UPD" tab = "DSFCONTRPLAN_DBT" pkcol = "T_ID" pk = "' || CAST(:NEW.T_ID AS VARchar2) || '"/>';
      ELSE
         msgmeta := '<XML action = "INS" tab = "DSFCONTRPLAN_DBT" pkcol = "T_ID" pk = "' || CAST(:NEW.T_ID AS VARchar2) || '"/>';
      END IF;
   ELSE  msgmeta := '<XML action = "DEL" tab = "DSFCONTRPLAN_DBT" pkcol = "T_ID" pk = "' || CAST(:OLD.T_ID AS VARchar2) || '"/>';
   END IF;
