CREATE OR REPLACE TRIGGER "TR_DDLCONTR_DBT_CDC"
...
BEGIN
   ...
   IF :NEW.T_DLCONTRID IS NOT NULL THEN
      IF :OLD.T_DLCONTRID IS NOT NULL THEN
         msgmeta := '<XML action = "UPD" tab = "DDLCONTR_DBT" pkcol = "T_DLCONTRID" pk = "' || CAST(:NEW.T_DLCONTRID AS VARchar2) || '"/>';
      ELSE
         msgmeta := '<XML action = "INS" tab = "DDLCONTR_DBT" pkcol = "T_DLCONTRID" pk = "' || CAST(:NEW.T_DLCONTRID AS VARchar2) || '"/>';
      END IF;
   ELSE  msgmeta := '<XML action = "DEL" tab = "DDLCONTR_DBT" pkcol = "T_DLCONTRID" pk = "' || CAST(:OLD.T_DLCONTRID AS VARchar2) || '"/>';
   END IF;
