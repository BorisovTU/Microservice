CREATE OR REPLACE TRIGGER "TR_DSFCONTR_DBT_CDC"
  FOR update or delete or insert
   OF t_number,t_datebegin,t_dateclose,t_servkind,t_servkindsub
   ON DSFCONTR_DBT
  compound trigger
  after each row is
...
BEGIN
   ...
   IF :NEW.T_ID IS NOT NULL THEN
      IF :OLD.T_ID IS NOT NULL THEN
         msgmeta := '<XML action = "UPD" tab = "DSFCONTR_DBT" pkcol = "T_ID" pk = "' || CAST(:NEW.T_ID AS VARchar2) || '"/>';
      ELSE
         msgmeta := '<XML action = "INS" tab = "DSFCONTR_DBT" pkcol = "T_ID" pk = "' || CAST(:NEW.T_ID AS VARchar2) || '"/>';
      END IF;
   ELSE  msgmeta := '<XML action = "DEL" tab = "DSFCONTR_DBT" pkcol = "T_ID" pk = "' || CAST(:OLD.T_ID AS VARchar2) || '"/>';
   END IF;
