CREATE OR REPLACE TRIGGER "TR_DCONTACT_DBT_CDC"
  FOR update or delete or insert
   OF T_CONTACTTYPE,T_VALUE,T_ISMAIN
   ON DCONTACT_DBT
  compound trigger
  after each row is
...
BEGIN
   ...
   IF :NEW.T_RECID IS NOT NULL THEN
      IF :OLD.T_RECID IS NOT NULL THEN
         msgmeta := '<XML action = "UPD" tab = "DCONTACT_DBT" pkcol = "T_RECID" pk = "' || CAST(:NEW.T_RECID AS VARchar2) || '"/>';
      ELSE
         msgmeta := '<XML action = "INS" tab = "DCONTACT_DBT" pkcol = "T_RECID" pk = "' || CAST(:NEW.T_RECID AS VARchar2) || '"/>';
      END IF;
   ELSE  msgmeta := '<XML action = "DEL" tab = "DCONTACT_DBT" pkcol = "T_RECID" pk = "' || CAST(:OLD.T_RECID AS VARchar2) || '"/>';
   END IF;
