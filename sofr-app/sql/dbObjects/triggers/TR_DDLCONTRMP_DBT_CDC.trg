CREATE OR REPLACE TRIGGER "TR_DDLCONTRMP_DBT_CDC"
  FOR update or delete or insert
   OF t_marketid,t_mpcode
   ON DDLCONTRMP_DBT
  compound trigger
  after each row is
...
BEGIN
   ...
   IF :NEW.T_ID IS NOT NULL THEN
      IF :OLD.T_ID IS NOT NULL THEN
         msgmeta := '<XML action = "UPD" tab = "DDLCONTRMP_DBT" pkcol = "T_ID" pk = "' || CAST(:NEW.T_ID AS VARchar2) || '"/>';
      ELSE
         msgmeta := '<XML action = "INS" tab = "DDLCONTRMP_DBT" pkcol = "T_ID" pk = "' || CAST(:NEW.T_ID AS VARchar2) || '"/>';
      END IF;
   ELSE  msgmeta := '<XML action = "DEL" tab = "DDLCONTRMP_DBT" pkcol = "T_ID" pk = "' || CAST(:OLD.T_ID AS VARchar2) || '"/>';
   END IF;
