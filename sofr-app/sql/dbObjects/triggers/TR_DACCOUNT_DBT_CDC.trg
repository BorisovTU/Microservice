CREATE OR REPLACE TRIGGER "TR_DACCOUNT_DBT_CDC"
  FOR update or delete or insert
   OF t_account,t_code_currency,t_open_date,t_close_date
   ON DACCOUNT_DBT
  compound trigger
  after each row is
...
BEGIN
   ...
   IF :NEW.T_ACCOUNTID IS NOT NULL THEN
      IF :OLD.T_ACCOUNTID IS NOT NULL THEN
         msgmeta := '<XML action = "UPD" tab = "DACCOUNT_DBT" pkcol = "T_ACCOUNTID" pk = "' || CAST(:NEW.T_ACCOUNTID AS VARchar2) || '"/>';
      ELSE
         msgmeta := '<XML action = "INS" tab = "DACCOUNT_DBT" pkcol = "T_ACCOUNTID" pk = "' || CAST(:NEW.T_ACCOUNTID AS VARchar2) || '"/>';
      END IF;
   ELSE  msgmeta := '<XML action = "DEL" tab = "DACCOUNT_DBT" pkcol = "T_ACCOUNTID" pk = "' || CAST(:OLD.T_ACCOUNTID AS VARchar2) || '"/>';
   END IF;
