CREATE OR REPLACE TRIGGER "TR_DPERSNIDC_DBT_CDC"
  FOR update or delete or insert
   OF T_PAPERSERIES,T_PAPERNUMBER,T_PAPERISSUEDDATE,T_PAPERISSUER,T_PAPERISSUERCODE,T_ISMAIN,T_ISNOTVALID,T_VALIDTODATE
   ON DPERSNIDC_DBT
  compound trigger
  after each row is
...
BEGIN
   ...
   IF :NEW.T_PERSONID IS NOT NULL THEN
      IF :OLD.T_PERSONID IS NOT NULL THEN
         msgmeta := '<XML action = "UPD" tab = "DPERSNIDC_DBT" pkcol = "T_PERSONID" pk = "' || CAST(:NEW.T_PERSONID AS VARchar2) || '"/>';
      ELSE
         msgmeta := '<XML action = "INS" tab = "DPERSNIDC_DBT" pkcol = "T_PERSONID" pk = "' || CAST(:NEW.T_PERSONID AS VARchar2) || '"/>';
      END IF;
   ELSE  msgmeta := '<XML action = "DEL" tab = "DPERSNIDC_DBT" pkcol = "T_PERSONID" pk = "' || CAST(:OLD.T_PERSONID AS VARchar2) || '"/>';
   END IF;
