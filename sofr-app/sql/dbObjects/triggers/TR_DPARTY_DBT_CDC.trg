CREATE OR REPLACE TRIGGER "TR_DPARTY_DBT_CDC"
...
BEGIN
   ...
   IF :NEW.T_PARTYID IS NOT NULL THEN
      IF :OLD.T_PARTYID IS NOT NULL THEN
         msgmeta := '<XML action = "UPD" tab = "DPARTY_DBT" pkcol = "T_PARTYID" pk = "' || CAST(:NEW.T_PARTYID AS VARchar2) || '"/>';
      ELSE
         msgmeta := '<XML action = "INS" tab = "DPARTY_DBT" pkcol = "T_PARTYID" pk = "' || CAST(:NEW.T_PARTYID AS VARchar2) || '"/>';
      END IF;
   ELSE  msgmeta := '<XML action = "DEL" tab = "DPARTY_DBT" pkcol = "T_PARTYID" pk = "' || CAST(:OLD.T_PARTYID AS VARchar2) || '"/>';
   END IF;
