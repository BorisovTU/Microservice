      ...
      if v_avoir_amount.count > 0 then
         FOR i IN v_avoir_amount.FIRST .. v_avoir_amount.LAST LOOP
            ...
            if v_rate > 0 then 
               ...
               avoir_amount_rec.Amount := RSI_RSB_FIInstr.ConvSum(avoir_amount_rec.AmountCur, v_avoir_amount(i).fiid_rate, RSI_RSB_FIInstr.NATCUR, datax.OperationDate);
               avoir_amount_rec.AmountCur := RSI_RSB_FIInstr.ConvSum(avoir_amount_rec.AmountCur, v_avoir_amount(i).fiid_rate, v_fiid_table, datax.OperationDate);
            end if;
