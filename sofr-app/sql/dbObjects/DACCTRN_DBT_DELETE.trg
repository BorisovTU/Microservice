...
DECLARE
    ...
    v_uTableProcessEvent uTableProcessEvent_dbt%ROWTYPE;
BEGIN
    ...
    BEGIN
        ...
        SELECT A.T_RECID, A.T_OBJECTID, A.T_STATUS, A.T_TIMESTAMP, A.T_TYPE
         ...
         AND (DELETING or ( UPDATING  AND :new.T_STATE = 4))
          THEN
            v_uTableProcessEvent.T_STATUS := OBJECT_STATUS_DEL;
            v_uTableProcessEvent.T_TIMESTAMP := SYSDATE;
            v_uTableProcessEvent.T_RECID := v_RecId;
            trgpckg_dacctrn_dbt.v_tableprocessevent(trgpckg_dacctrn_dbt.v_tableprocessevent.count) := v_uTableProcessEvent;
