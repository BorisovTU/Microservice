create or replace package body nontrading_orders_utils as
  ...
  begin
    ...
    then
      rsb_payments_api.InsertEmailNotify(p_EmailGroup => nontrading_orders_read.get_email_grp_kafka_err,
                                         p_Head       => p_head,
                                         p_Text       => p_text);
    end if;
  end send_error_mail;
  ...
  procedure save_nptxop_req (
    p_req nontrading_orders_buffer%rowtype
  ) is
  begin
    insert into nontrading_orders_buffer values p_req;
  end save_nptxop_req;
  ...
  function save_transfer_to_buf (
    ...
    l_error_id      nontrading_orders_buffer.error_id%type := nontrading_orders_read.buf_err_no_error;
    l_status_id     nontrading_orders_buffer.status_id%type := nontrading_orders_read.buf_status_ready;
    l_nptx_req_row  nontrading_orders_buffer%rowtype;
    l_src           nontrading_orders_buffer.src%type;
  begin
    l_src := nontrading_orders_read.get_buf_by_incoming_src_name(p_incoming_name => p_src);
    if nontrading_orders_read.get_active_req(p_src    => l_src
                                            ,p_ext_id => p_ext_id
                                            ,p_kind   => nontrading_orders_read.buf_kind_transfer
                                            ,p_client => p_client_cft_id) is not null
    then
      l_error_id := nontrading_orders_read.buf_err_duplicate;
      l_status_id := nontrading_orders_read.buf_status_error;
    end if;
    
    l_nptx_req_row.req_id                 := nontrading_orders_buffer_seq.nextval;
    l_nptx_req_row.src                    := l_src;
    l_nptx_req_row.external_id            := p_ext_id;
    l_nptx_req_row.client_cft_id          := p_client_cft_id;
    l_nptx_req_row.contract               := p_contract;
    l_nptx_req_row.marketplace_withdrawal := p_marketplace;
    l_nptx_req_row.is_full_rest           := p_is_full_rest;
    l_nptx_req_row.currency               := p_currency;
    l_nptx_req_row.amount                 := p_amount;
    l_nptx_req_row.marketplace_enroll     := p_marketplace_enroll;
    ...
    l_nptx_req_row.kind                   := nontrading_orders_read.buf_kind_transfer;
  ...
  exception
    when others then
      it_log.log_error(p_object => 'nontrading_orders_utils.save_transfer_to_buf',
                       p_msg    => sqlerrm);
      return 0;
  ...
  function save_out_to_buf (
    ...
    l_status_id     nontrading_orders_buffer.status_id%type := nontrading_orders_read.buf_status_ready;
    l_nptx_req_row  nontrading_orders_buffer%rowtype;
    l_kind          nontrading_orders_buffer.kind%type;
    l_src           nontrading_orders_buffer.src%type;
  begin
    l_kind := case when p_marketplace = 0 then nontrading_orders_read.buf_kind_out_otc else nontrading_orders_read.buf_kind_out_exchange end;
    l_src  := nontrading_orders_read.get_buf_by_incoming_src_name(p_incoming_name => p_src);
    if nontrading_orders_read.get_active_req(p_src    => l_src
                                            ,p_ext_id => p_ext_id
                                            ,p_kind   => l_kind
  ...
  exception
    when others then
      it_log.log_error(p_object => 'nontrading_orders_utils.save_out_to_buf',
                       p_msg    => sqlerrm);
      return 0;
  end save_out_to_buf;
  ...
  begin
    update nontrading_orders_buffer r
       set r.error_id  = p_error_id,
           r.status_id = nontrading_orders_read.buf_status_error
  ...
  begin
    update nontrading_orders_buffer r
       set r.status_id = nontrading_orders_read.buf_status_wait,
           r.status_changed = systimestamp
     where r.operation_id = p_nptxop_id;
  end set_status_wait;
  ...
  begin
    update nontrading_orders_buffer r
       set r.status_id = nontrading_orders_read.buf_status_done,
           r.status_changed = systimestamp
     where r.operation_id = p_nptxop_id;
  end set_status_done;
  ...
  begin
    update nontrading_orders_buffer r
       set r.status_id = nontrading_orders_read.buf_status_reject,
           r.status_changed = systimestamp
     where r.operation_id = p_nptxop_id;
    
    nontrading_orders_utils.send_order_status(p_operid  => p_nptxop_id,p_status => nontrading_orders_read.buf_status_reject );
  ...
  begin
    update nontrading_orders_buffer r
       set r.status_id = nontrading_orders_read.buf_status_deleted,
           r.status_changed = systimestamp
     where r.operation_id = p_nptxop_id;
  ...
  begin
    ...
    values (dnptxop_dbt_seq.nextval,
            nontrading_orders_read.dockind,
            2037,
            p_subkind,
            trunc(sysdate),
            1,
  ...
  begin
    l_rqacc_row.t_dockind          := nontrading_orders_read.dockind;
    ...
    l_rqacc_row.t_bankcode         := nontrading_orders_read.get_party_code(p_party_id => l_rqacc_row.t_bankid, p_code_kind => l_rqacc_row.t_bankcodekind);
    l_rqacc_row.t_bankname         := nontrading_orders_read.get_party_name(p_party_id => l_rqacc_row.t_bankid);
    l_rqacc_row.t_dockind          := nontrading_orders_read.dockind;
  ...
  begin
    insert into ddlrq_dbt (t_dockind,
                           ...
                           t_changedate,
                           t_version)
    values (nontrading_orders_read.dockind,
            p_nptxop_id,
            1,
            0,
            0,
  ...
  procedure save_payment( p_documentid        dnptxop_dbt.t_id%type,
                          ...
                          p_outsum            dnptxop_dbt.t_outsum%type,
                          p_dep_receiver      ddp_dep_dbt.t_code%type,
  ...
  begin 
    ...
    l_payerbankid := payment_utils.get_department_param(p_dep_code => 1,
                                                        o_bankname => l_payerbankname,
                                                        o_coracc   => l_payercorracc);
    l_receiverbankid := payment_utils.get_department_param(p_dep_code => p_dep_receiver,
    ...
    l_clientname := nontrading_orders_read.get_party_name(p_party_id => p_client);
    l_clientinn := nvl(nontrading_orders_read.get_party_code(p_party_id => p_client, p_code_kind => 16), chr(1));
    l_ground := 'Списание денежных средств по операции № '||p_code;
    
    l_paymentid := payment_utils.save_pmpaym (p_documentid      => p_documentid,
                                              p_dockind         => nontrading_orders_read.dockind,
                                              p_purpose         => 1,
                                              p_subpurpose      => 0,
  ...
  begin
    spground_utils.save_ground(pio_spgroundid => l_spground_id,
                               ...
                               p_kind         => p_groundkind,
                               p_direction    => 1,
                               p_registrdate  => p_date,
                               p_registrtime  => p_time,
                               p_xld          => p_code,
                               p_altxld       => p_extcode,
                               p_signeddate   => p_signdate,
                               p_backoffice   => 'S',
                               p_party        => p_partyid,
                               p_partyname    => party_read.get_party_name(p_partyid => p_partyid),
                               p_partycode    => nvl(party_read.get_party_code(p_party_id => p_partyid, p_code_kind => 1), chr(1)),
                               p_methodapplic => case when p_src = 'ЕФР' then 2 else 1 end);

    spground_utils.link_spground_to_doc(p_sourcedockind => nontrading_orders_read.dockind,
                                        p_sourcedocid   => p_nptxop_id,
                                        p_spgroundid    => l_spground_id);
  end create_spground;

  procedure create_spground_client_order (
    p_nptxop_id  dnptxop_dbt.t_id%type,
    p_code       dnptxop_dbt.t_code%type,
    p_partyid    dnptxop_dbt.t_client%type,
    p_date       nontrading_orders_buffer.req_date%type,
  ...
  begin
    create_spground(p_nptxop_id  => p_nptxop_id,
                    p_groundkind => 251,
                    p_code       => p_code,
                    p_extcode    => p_code,
                    p_partyid    => p_partyid,
                    ...
                    p_signdate   => p_date,
                    p_src        => p_src);
  end create_spground_client_order;
  ...
  begin
    if nontrading_orders_read.is_allowed_autorun(p_system_name          => p_req_row.src,
                                                 p_exchange_type        => p_req_row.marketplace_withdrawal,
                                                 p_exchange_type_target => p_req_row.marketplace_enroll,
                                                 p_is_full_rest         => p_req_row.is_full_rest,
                                                 p_currency             => p_req_row.currency,
                                                 p_iis                  => p_iis) = 0
    then
      it_log.log(p_msg => 'req_id = ' || p_req_row.req_id || '; autorun is not allowed by config');
      return;
    end if;

    if nontrading_orders_read.is_client_allowed_autorun(p_client_id         => p_client_id,
                                                        p_exclude_oper_list => t_number_list(p_op_id)) = 0
    then
      it_log.log(p_msg => 'req_id = ' || p_req_row.req_id || '; client autorun is not allowed');
      return;
    end if;
    
    funcobj_utils.save_task(p_objectid => p_op_id,
                            p_funcid   => funcobj_utils.get_func_id(p_code => nontrading_orders_read.get_funcobj_code),
    ...
    set_buf_status(p_req_id    => p_req_row.req_id,
                   p_status_id => nontrading_orders_read.buf_status_executing);
  ...
  begin
    funcobj_utils.delete_active_task(p_objectid => p_op_id,
                                     p_funcid   => funcobj_utils.get_func_id(p_code => nontrading_orders_read.get_funcobj_code));

    set_buf_status(p_req_id    => p_req_id,
                   p_status_id => nontrading_orders_read.buf_status_wait);
    commit;
  end delete_from_auto_executing;
  
  procedure set_src_categ (
    p_op_id        dnptxop_dbt.t_id%type,
    p_src_buf_name nontrading_orders_source.buffer_name%type
  ) is
  begin
    categ_utils.add_new(p_object_type => nontrading_orders_read.order_object_type,
                        p_object      => lpad(p_op_id, 34, '0'),
                        p_group_id    => nontrading_orders_read.categ_group_src,
                        p_attr_id     => nontrading_orders_read.get_src_attr_by_name(p_src_buf_name => p_src_buf_name));
  end set_src_categ;
  ...
  begin
    l_nptxop_id := save_nptxop(
      p_subkind       => nontrading_orders_read.subkind_transfer,
      p_code          => p_code,
  ...
  begin
    ...
    if p_account_tgt is null then
      ...
      return nontrading_orders_read.buf_err_acc_enrl_not_found;
    end if;
    
    if p_ekk is null then
      it_log.log(p_msg => 'req_id = ' || p_req_id || '; Ekk not found; contractid = ' || p_contract_tgt);
      return nontrading_orders_read.buf_err_ekk_not_found;
    end if;

    if p_marketplace not in (nontrading_orders_read.buf_otc_market, nontrading_orders_read.buf_exchange_market) and nontrading_orders_read.is_edp_subcontr (p_contract) then
      it_log.log(p_msg => 'req_id = ' || p_req_id || '; Contract INST is EDP; contractid = ' || p_contract);
      return nontrading_orders_read.buf_err_cntr_is_edp;
    end if;

    if p_marketplace_enroll not in (nontrading_orders_read.buf_otc_market, nontrading_orders_read.buf_exchange_market) and nontrading_orders_read.is_edp_subcontr (p_contract_tgt) then
      it_log.log(p_msg => 'req_id = ' || p_req_id || '; Contract INST is EDP; contractid = ' || p_contract_tgt);
      return nontrading_orders_read.buf_err_cntr_is_edp;
    end if;

    if not nontrading_orders_read.check_party_sfcontr(p_sfcontr_id => p_contract, p_party_id => p_client) then
      it_log.log(p_msg => 'req_id = ' || p_req_id || '; party_id not matched with contract; contractid = ' || p_contract || '; partyid = ' || p_client);
      return nontrading_orders_read.buf_err_clnt_not_matched_w_cntr;      
    end if;

    if not nontrading_orders_read.check_party_sfcontr(p_sfcontr_id => p_contract_tgt, p_party_id => p_client) then
      it_log.log(p_msg => 'req_id = ' || p_req_id || '; party_id not matched with contract; contractid = ' || p_contract_tgt || '; partyid = ' || p_client);
      return nontrading_orders_read.buf_err_clnt_not_matched_w_cntr;      
    end if;

    return nontrading_orders_read.buf_err_no_error;
  end check_data_transfer;
  
  function save_transfer_by_buf (
    p_req_row  nontrading_orders_buffer%rowtype
  ) return number is
    l_log_object      varchar2(100) := 'nontrading_orders_utils.save_transfer_by_buf';
    ...
    l_nptxop_id       dnptxop_dbt.t_id%type;
    l_error_id        nontrading_orders_error.error_id%type;
    l_ekk             varchar2(50);
    l_iis             number(1);
  begin
    nontrading_orders_read.get_buf_data_transfer(p_req_row       => p_req_row,
    ...
    l_ekk := sfcontr_read.get_ekk_contr(p_sfcontr_id => l_contract);
    ...
    l_error_id := check_data_transfer(p_req_id             => p_req_row.req_id,
                                      ...
                                      p_ekk                => l_ekk,
                                      p_marketplace        => p_req_row.marketplace_withdrawal,
                                      p_marketplace_enroll => p_req_row.marketplace_enroll);
    if l_error_id != nontrading_orders_read.buf_err_no_error then
      save_error(p_req_id => p_req_row.req_id, p_error_id => l_error_id);
      
      if l_error_id != nontrading_orders_read.buf_err_duplicate then
        send_error_mail(p_head => 'Произошла ошибка в работе информационного сервиса неторговых поручений. Не удалось загрузить неторговое поручение в СОФР',
                        p_text => 'Произошла ошибка при попытке завести операцию по поручению: ' || nontrading_orders_read.get_error_text(p_error_id => l_error_id) || '. '
    ...
    l_code := user_NPTXFUNC.GenerateOpCodebySyq(pEKK  => l_ekk,
                                                pYear => to_char(sysdate, 'yyyy'),
                                                pDBO  => nontrading_orders_read.get_src_opername_by_buf(p_buf_name => p_req_row.src));
    ...
    l_nptxop_id := save_transfer(
      ...
      p_account_tgt   => l_account_tgt,
      p_outsum        => l_outsum,
      p_placekind     => case p_req_row.marketplace_withdrawal when nontrading_orders_read.buf_otc_market then nontrading_orders_read.bank_place_kind else nontrading_orders_read.rc_place_kind end,
      p_place         => case p_req_row.marketplace_withdrawal when nontrading_orders_read.buf_otc_market then nontrading_orders_read.otc_place else nontrading_orders_read.nko_nrd_place end,
      p_marketplace   => case p_req_row.marketplace_withdrawal 
                           when nontrading_orders_read.buf_exchange_market then nontrading_orders_read.get_micex_id 
                           when nontrading_orders_read.buf_spb_market      then nontrading_orders_read.get_spbex_id 
                           when nontrading_orders_read.buf_currency_market then nontrading_orders_read.get_nrd_id
                           when nontrading_orders_read.buf_forts_market    then nontrading_orders_read.get_nrd_id 
                           else 0
                         end,
      p_marketsector  => case p_req_row.marketplace_withdrawal 
                           when nontrading_orders_read.buf_exchange_market then nontrading_orders_read.main_sector 
                           when nontrading_orders_read.buf_forts_market    then nontrading_orders_read.forts_sector 
                           when nontrading_orders_read.buf_currency_market then nontrading_orders_read.cur_sector
                           else 0 
                         end,
      p_placekind2    => case p_req_row.marketplace_enroll 
                           when nontrading_orders_read.buf_exchange_market then nontrading_orders_read.rc_place_kind 
                           when nontrading_orders_read.buf_currency_market then nontrading_orders_read.rc_place_kind
                           when nontrading_orders_read.buf_forts_market    then nontrading_orders_read.rc_place_kind
                           when nontrading_orders_read.buf_spb_market      then nontrading_orders_read.rc_place_kind
                           else nontrading_orders_read.bank_place_kind 
                         end,
      p_place2        => case p_req_row.marketplace_enroll when nontrading_orders_read.buf_otc_market then nontrading_orders_read.otc_place else nontrading_orders_read.nko_nrd_place end,
      p_marketplace2  => case p_req_row.marketplace_enroll 
                           when nontrading_orders_read.buf_exchange_market then nontrading_orders_read.get_micex_id 
                           when nontrading_orders_read.buf_spb_market      then nontrading_orders_read.get_spbex_id 
                           when nontrading_orders_read.buf_currency_market then nontrading_orders_read.get_nrd_id
                           when nontrading_orders_read.buf_forts_market    then nontrading_orders_read.get_nrd_id 
      ...
      p_marketsector2 => case p_req_row.marketplace_enroll 
                           when nontrading_orders_read.buf_exchange_market then nontrading_orders_read.main_sector 
                           when nontrading_orders_read.buf_forts_market    then nontrading_orders_read.forts_sector 
                           when nontrading_orders_read.buf_currency_market then nontrading_orders_read.cur_sector
                           else 0 
    ...
    create_spground_client_order(p_nptxop_id => l_nptxop_id,
                                 p_code      => l_code,
                                 p_partyid   => l_client,
                                 p_date      => p_req_row.req_date,
                                 p_time      => p_req_row.req_time,
                                 p_src       => p_req_row.src);

    if p_req_row.marketplace_withdrawal = nontrading_orders_read.buf_otc_market then
      note_utils.add_new(p_object_type => l_object_type,
                         p_document_id => lpad(l_nptxop_id, 34, '0'),
                         p_note_kind   => 104,
                         p_value       => nontrading_orders_read.note_enroll_allowed);
    ...
    set_src_categ(p_op_id        => l_nptxop_id,
                  p_src_buf_name => p_req_row.src);
    ...
    push_to_auto_executing(p_req_row   => p_req_row,
                           p_op_id     => l_nptxop_id,
  ...
  exception
    when others then
      rollback;
      save_error(p_req_id => p_req_row.req_id, p_error_id => nontrading_orders_read.buf_err_internal_error);
      send_error_mail(p_head => 'Произошла ошибка в работе информационного сервиса неторговых поручений. Не удалось загрузить неторговое поручение в СОФР',
                      p_text => 'Произошла ошибка при попытке завести операцию по поручению: ' || nontrading_orders_read.get_error_text(p_error_id => nontrading_orders_read.buf_err_internal_error) || '. '
                             || 'Поручение доступно для просмотра в интерфейсе Буфера Неторговых Поручений, после исправления можно инициировать повторную попытку заведения операции');
  ...
  begin
    l_account_tax := case
                       when party_read.is_legal_entity_clear(p_party_id => p_client) = 1 then
                         ...
                         nontrading_orders_read.get_sfcontr_account(p_sfcontr_id => p_contract, p_fiid => 0, p_date => trunc(sysdate))
                     end;

    l_nptxop_id := save_nptxop(
      p_subkind       => nontrading_orders_read.subkind_out,
      p_code          => p_code,
      p_contract      => p_contract,
      p_client        => p_client,
      p_currency      => p_currency,
      ...
      p_marketsector2 => 0,
      p_calcndfl      => chr(0),
      p_flagtax       => case p_src when 'ДБО ЮЛ' then chr(0) else chr(88) end, 
  ...
  function check_data_out (
    ...
    p_ekk          varchar2,
    p_marketplace  nontrading_orders_buffer.marketplace_withdrawal%type,
    p_dep_id       dnptxop_dbt.t_department%type
  ) return nontrading_orders_error.error_id%type is
  begin
    if p_contract is null then
      it_log.log(p_msg => 'req_id = ' || p_req_id || '; Contract not found');
    ...
    if p_ekk is null then
      it_log.log(p_msg => 'req_id = ' || p_req_id || '; Ekk not found; contractid = ' || p_contract);
      return nontrading_orders_read.buf_err_ekk_not_found;
    end if;
    ...
    if p_dep_id is null then
      it_log.log(p_msg => 'req_id = ' || p_req_id || '; Department not found');
      return nontrading_orders_read.buf_err_department_not_found;
    end if;
  ...
  function save_out_by_buf (
    ...
    l_log_object      varchar2(100) := 'nontrading_orders_utils.save_out_otc_by_buf';
    l_object_type     number(3) := 131;
    l_contract        dnptxop_dbt.t_contract%type;
    l_client          dnptxop_dbt.t_client%type;
    l_currency        dnptxop_dbt.t_currency%type;
    ...
    l_outsum          dnptxop_dbt.t_outsum%type;
    l_code            dnptxop_dbt.t_code%type;
    l_dep_id          dnptxop_dbt.t_department%type;
    l_nptxop_id       dnptxop_dbt.t_id%type;
    ...
    l_iis             number(1);
  begin
    nontrading_orders_read.get_buf_data_out(p_req_row   => p_req_row,
                                            po_contract => l_contract,
                                            po_currency => l_currency,
                                            po_account  => l_account,
                                            po_outsum   => l_outsum,
    ...
    l_client := party_read.get_id_by_cft_code(p_cft_code => p_req_row.client_cft_id);
    l_ekk := sfcontr_read.get_ekk_subcontr(p_sfcontr_id => l_contract);
    l_dep_id := nontrading_orders_read.get_dep_id_by_code(p_code => substr(p_req_row.department, 1, 2) || '00');
    ...
    l_nptxop_id := save_out(
      ...
      p_outsum        => l_outsum,
      p_placekind     => nontrading_orders_read.bank_place_kind,
      p_place         => l_place,
      p_dep_code      => l_dep_id,
      p_iis           => l_iis,
      p_src           => p_req_row.src
  ...
  function save_operation_by_buf (
    ...
    l_log_object      varchar2(100) := 'nontrading_orders_utils.save_operation_by_buf';
  begin
    ...
    l_req_row := nontrading_orders_read.get_req_row(p_req_id => p_req_id);
    ...
    --nontrading_orders_read.buf_status_ready - from ui
    --nontrading_orders_read.buf_status_creating - from get_req_to_work
    if l_req_row.status_id not in (nontrading_orders_read.buf_status_ready, nontrading_orders_read.buf_status_creating) then
      it_log.log_error(p_object => l_log_object,
    ...
    return case l_req_row.kind
              when nontrading_orders_read.buf_kind_out_otc      then save_out_by_buf      (p_req_row => l_req_row)
              when nontrading_orders_read.buf_kind_out_exchange then save_out_by_buf      (p_req_row => l_req_row)
              when nontrading_orders_read.buf_kind_transfer     then save_transfer_by_buf (p_req_row => l_req_row)
            else
              0
            end;
  ...
  begin
    if p_marketplace_enroll is not null then
      l_req_id := nontrading_orders_utils.save_transfer_to_buf(p_src                => p_src
                                                              ,p_ext_id             => p_ext_id
                                                              ...
                                                              ,p_currency           => p_currency
                                                              ,p_amount             => p_amount
                                                              ,p_marketplace_enroll => p_marketplace_enroll
                                                              ,p_req_date           => p_req_date
                                                              ,p_req_time           => p_req_time
                                                              ,p_file_name          => p_file_name);
    else
      if p_department is null then
        it_log.log_error(p_object => 'nontrading_orders_utils.process_req_from_json',
                         p_msg    => 'Error: department required. src=' || p_src || '; ext_id=' || p_ext_id);
        return 1;
      end if;
      l_req_id := nontrading_orders_utils.save_out_to_buf(p_src           => p_src
                                                         ,p_ext_id        => p_ext_id
                                                         ,p_client_cft_id => p_client_cft_id
                                                         ,p_iis           => p_iis
                                                         ,p_contract      => p_contract
    ...
    nontrading_orders_read.get_allowed_period(p_date    => trunc(sysdate),
                                              po_dbegin => l_dbegin,
                                              po_dend   => l_dend);
    ...
    if rsi_rsbcalendar.IsWorkDay(p_Date => sysdate) = 1 and
      ...
      l_result := save_operation_by_buf(p_req_id => l_req_id); 
    end if;
    
    return case when l_req_id > 0 then 0 else 1 end;
  exception
    when others then
      return 1;
  end process_req;
  ...
  begin
    update nontrading_orders_buffer r
       set r.status_id = nontrading_orders_read.buf_status_creating
     where r.rowid = (select rowid
                        from nontrading_orders_buffer
                       where 0 = case when error_id = 0 and operation_id is null then error_id end --index ind_nptx_req_to_create
                         and status_id = nontrading_orders_read.buf_status_ready
                        order by req_date, req_time
                        fetch next 1 rows only)
       and r.status_id = nontrading_orders_read.buf_status_ready
  ...
  exception
    when others then
      rollback;
      it_log.log_error(p_object => 'nontrading_orders_utils.get_req_to_work',
                       p_msg    => 'req_id = ' || l_req_id || '. Error: ' || sqlerrm);
      return null;
  end get_req_to_work;
  
  procedure create_all_operations_from_buf is
    l_cnt        number(9) := 0;
  ...
  exception
    when others then
      it_log.log_error(p_object => 'nontrading_orders_utils.create_all_operations_from_buf',
                       p_msg    => 'Error: ' || sqlerrm);
  end create_all_operations_from_buf;
  
  procedure set_last_feedback (
    p_oper_id nontrading_orders_buffer.operation_id%type,
    p_time    nontrading_orders_buffer.last_feed_back%type
  ) is
  begin
    update nontrading_orders_buffer b
       set b.last_feed_back = p_time
     where b.operation_id = p_oper_id;

    note_utils.save_note(p_object_type => nontrading_orders_read.order_object_type,
                         p_note_kind   => nontrading_orders_read.notekind_last_sent_status,
                         p_document_id => lpad(p_oper_id, 34, '0'),
                         p_note        => to_char(p_time, 'dd.mm.yyyy hh24:mi:ss'),
                         p_date        => trunc(sysdate));
  end set_last_feedback;
  ...
  begin
    case p_proc
      when 'SAVE_REQ_FROM_JSON' then
        if it_rs_interface.get_parm_varchar_path(p_parm_path => nontrading_orders_read.GC_PARAM_LOAD_NOTIFICATION_OD) = chr(88)
        then
          nontrading_orders_utils.send_error_mail(p_head => 'СОФР. Ошибка загрузки неторгового поручения'
                                                 ,p_text => 'Произошла ошибка в работе информационного сервиса неторговых поручений. ' || p_errtxt);
        end if;
        if p_nosupport = 0
           and it_rs_interface.get_parm_varchar_path(p_parm_path => nontrading_orders_read.GC_PARAM_LOAD_NOTIFICATION_SUPPORT) = chr(88)
        then
          it_event.AddErrorITLogMonitoring(p_SystemId => v_SystemId, p_ServiceName => 'SendNonTradingOrder', p_ErrorCode => p_errcode, p_ErrorDesc => p_errtxt, p_LevelInfo => 8);
        end if;
      when 'SEND_ORDER_STATUS' then
        if it_rs_interface.get_parm_varchar_path(p_parm_path => nontrading_orders_read.GC_PARAM_STATUS_NOTIFICATION_OD) = chr(88)
        then
          nontrading_orders_utils.send_error_mail(p_head => 'СОФР. Ошибка отправки статуса обработки неторгового поручения во фронтальную систему'
                                                 ,p_text => 'Произошла ошибка в работе информационного сервиса неторговых поручений. ' || p_errtxt);
        end if;
        if p_nosupport = 0
           and it_rs_interface.get_parm_varchar_path(p_parm_path => nontrading_orders_read.GC_PARAM_STATUS_NOTIFICATION_SUPPORT) = chr(88)
        then
          it_event.AddErrorITLogMonitoring(p_SystemId => v_SystemId, p_ServiceName => 'SendNonTradingOrderStatusReg', p_ErrorCode => p_errcode, p_ErrorDesc => p_errtxt, p_LevelInfo => 8);
        end if;
      when 'RESP_STATUS_MESSAGE' then
        if it_rs_interface.get_parm_varchar_path(p_parm_path => nontrading_orders_read.GC_PARAM_STATUS_NOTIFICATION_OD) = chr(88)
        then
          nontrading_orders_utils.send_error_mail(p_head => 'СОФР. Получена ошибка обработки статуса неторгового поручения из фронтальной системы'
                                                 ,p_text => 'Произошла ошибка в работе информационного сервиса неторговых поручений. ' || p_errtxt);
        end if;
        if p_nosupport = 0
           and it_rs_interface.get_parm_varchar_path(p_parm_path => nontrading_orders_read.GC_PARAM_STATUS_NOTIFICATION_SUPPORT) = chr(88)
        then
          it_event.AddErrorITLogMonitoring(p_SystemId => v_SystemId, p_ServiceName => 'SendNonTradingOrderStatusResp', p_ErrorCode => p_errcode, p_ErrorDesc => p_errtxt, p_LevelInfo => 8);
        end if;
    end case;
  ...
  procedure send_order_status_errtxt(p_operid    integer -- Идентификатор T_ID из DNPTXOP_DBT
    ...
    l_req_row        nontrading_orders_buffer%rowtype;
    l_src            varchar2(128);
    l_external_id    nontrading_orders_buffer.external_id%type;
    l_str            varchar2(4000);
    v_decline_reason nontrading_orders_error.error_name%type;
    v_clientid       dobjcode_dbt.t_code%type;
  begin
    o_ErrorCode := 0;
    l_req_row   := nontrading_orders_read.get_req_row(p_oper_id => p_operid);
    ...
    then
      ...
      l_src := nontrading_orders_read.get_src_buf_name_by_opername(p_opercode_name => l_str);
    ...
    if l_src is null
       or nontrading_orders_read.is_allowed_send_status(l_src) = 0
    then
      o_ErrorCode := 1;
    ...
    if o_ErrorCode = 0
    then
      if p_status = nontrading_orders_read.buf_status_reject
      then
        v_decline_reason := note_utils.GetTextID34(p_object_type => 131,
                                               p_id => p_operid,
                                               p_note_kind => 103);
      end if;
      set_last_feedback(p_oper_id => p_operid,
                        p_time    => systimestamp);
      it_frontsystems.form_status_message_json(p_system_origin  => l_src --  "ДБО ФЛ" или "ЕФР" в зависимости от источника
                                              ,p_status         => nontrading_orders_read.get_status_name(p_status) --  Текстовая расшифровка числового значения из send_order_status()
    ...
    then
      ...
      o_ErrorDesc := 'Сообщение о статусе <' || nontrading_orders_read.get_status_name(p_status) || '>' || case
                       when v_decline_reason is not null then
                        '(' || v_decline_reason || ')'
                     end || ' отправлено в ' || l_src;
    end if;
  exception
    when others then
      declare
