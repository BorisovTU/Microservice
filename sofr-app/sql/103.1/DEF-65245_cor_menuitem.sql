BEGIN
   DELETE FROM DMENUITEM_DBT
         WHERE T_ICASEITEM = 929
               AND (T_CSYSTEMITEM <> CHR (88) OR T_CSYSTEMITEM IS NULL);
END;
/
...
DECLARE
   ...
   C_CASEITEM            NUMBER := 929;  --Номер модуля
   C_NAME                VARCHAR2 (255) := 'Отчеты MFB82';
   ...
   BEGIN
      ...
      SELECT NVL (
                (SELECT NVL(MAX(T_INUMBERPOINT), 0)
                   FROM dmenuitem_dbt
                  WHERE     trim(replace(t_sznameitem, '~', '')) = 'Договоры'
      ...
      SELECT NVL (
                (SELECT NVL(MAX(T_INUMBERPOINT), 0)
                   FROM dmenuitem_dbt
                  WHERE     trim(replace(t_sznameitem, '~', '')) = 'Договоры брокерского обслуживания'
                        AND T_IPROGITEM = C_IDENTPROGRAM_CODE
                        AND T_ISTEMPLATE = p_istemplate
                        AND T_OBJECTID = p_numberoper
...
BEGIN
   ...
   FOR i IN (SELECT DISTINCT T_OPER
              ...
              WHERE ROL.T_ROLEID IN (10014, 10007))
   LOOP
