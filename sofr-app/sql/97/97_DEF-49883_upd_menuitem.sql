...
BEGIN
   DELETE FROM DMENUITEM_DBT WHERE (t_icaseitem = 20301 AND t_iidentprogram = 83) OR (t_icaseitem = 25125 AND t_iidentprogram = 158);
EXCEPTION
...
DECLARE
   ...
   BEGIN
      ...
      SELECT NVL (
                (SELECT NVL(MAX(T_INUMBERPOINT), 0)
                   FROM dmenuitem_dbt
                  WHERE     trim(t_sznameitem) in ('Регистры внутреннего учета','~Р~егистры внутреннего учета')
                        AND T_IPROGITEM = C_IDENTPROGRAM_CODE
                        AND T_ISTEMPLATE = p_istemplate
                        AND T_OBJECTID = p_numberoper
                        AND t_inumberfather = V_NUMBERFATHER),
