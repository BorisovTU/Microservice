...
DECLARE
   ...
   C_CASEITEM            NUMBER := 20203;  --Номер модуля
   C_NAME                VARCHAR2 (255) := 'Буферная таблица';
...
BEGIN
   ...
   FOR i IN (SELECT DISTINCT rol.T_OPER
              ...
              WHERE ROL.T_ROLEID IN (10010,10013))
   LOOP
...
DECLARE
   ...
   C_IDENTPROGRAM_CODE   NUMBER := 83; --Подсистема
   C_CASEITEM            NUMBER := 20204;  --Номер модуля
   C_NAME                VARCHAR2 (255) := 'Операции пересчета';
   V_NUMBERFATHER        NUMBER := 0;
