...
DECLARE
   ...
   C_CASEITEM            NUMBER := 20238;  --Номер модуля
   C_NAME                VARCHAR2 (255) := 'Таблица предельных значений СНОБ';
   ...
   BEGIN
      ...
      SELECT NVL (
                (SELECT NVL(MAX(T_INUMBERPOINT), 0)
                   FROM dmenuitem_dbt
                  WHERE     trim(replace(t_sznameitem, '~', '')) = 'Настройки для прогрессивной шкалы'
                        AND T_IPROGITEM = C_IDENTPROGRAM_CODE
                        AND T_ISTEMPLATE = p_istemplate
                        AND T_OBJECTID = p_numberoper
                        AND t_inumberfather = V_NUMBERFATHER),
