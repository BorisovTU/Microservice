...
declare
  ...
  /* 
                            ...
                            Ь Шлюз
  */
  menu_path       varchar2(256) := 'Сервисные операции'; -- путь к пункту меню c разделителем \
  menu_item       varchar2(128) := 'Подкрепление счетов НКЦ'; -- Имя пункта.
  menu_nameprompt varchar2(256) := 'Ручное подкрепление счетов клиентов в НКЦ'; -- Описание.
  usermodule_name varchar2(128) := 'Ручное подкрепление счетов клиентов в НКЦ'; -- Наименование пользовательского модуля 
  usermodule_file varchar2(128) := 'Refill_Scroll.mac'; -- Наименование макроса 
