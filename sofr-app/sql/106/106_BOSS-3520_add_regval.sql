...
DECLARE
  logID VARCHAR2(50) := 'BOSS-3520 Insert RegParm RegVal';
  v_ParentId NUMBER;
  v_cnt NUMBER := 0;
  v_ID  NUMBER := 0;
...
BEGIN
   ...
   IF (v_ParentId <> 0) THEN
     v_ID := AddRegParm(v_ParentId, 'ЗАГРУЗКА_НЕСКОЛЬКИХ_ДБО', 4, 'Возможность загрузки нескольких ДБО из ЦФТ по одному клиенту в разных сообщениях');
     AddRegVal(v_ID, 0, '');
   END IF;
