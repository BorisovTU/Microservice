...
DECLARE
  logID VARCHAR2(50) := 'BOSS-1468 Insert RegParm RegVal';
  v_ParentId NUMBER;
  v_cnt NUMBER := 0;
  v_ID  NUMBER := 0;
...
BEGIN
   ...
   BEGIN
     SELECT t_KeyId INTO v_ParentId FROM DREGPARM_DBT WHERE t_ParentId = v_ParentId AND LOWER(T_NAME) = LOWER('ИНВЕСТ_СОВЕТНИК');
     
   EXCEPTION WHEN NO_DATA_FOUND THEN 
       INSERT INTO DREGPARM_DBT 
          (T_KEYID, T_PARENTID, T_NAME, T_TYPE, T_GLOBAL, T_DESCRIPTION, T_SECURITY, T_ISBRANCH, T_TEMPLATE)
        VALUES 
          (0, v_ParentId, 'ИНВЕСТ_СОВЕТНИК', 0, CHR(0),CHR(1), CHR(0), CHR(0), CHR(1)) 
        RETURNING T_KEYID INTO v_ParentId;
   END;

   IF (v_ParentId <> 0) THEN
     v_ID := AddRegParm(v_ParentId, 'ДИРЕКТОРИЯ_КЛИЕНТСКИЙ_ОТЧЕТ', 2, 'Директория для сохранения отчетов для клиента');
     AddRegVal(v_ID, 0, '');
   END IF;
