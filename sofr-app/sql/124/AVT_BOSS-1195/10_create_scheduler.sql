  ...
  procedure create_service (
   ...
   begin
    ...
    insert into dss_func_dbt (t_id,
                              ...
                              t_timeout,
                              t_parameters)
    values (trunc(p_id*10, -3)*10 + mod(p_id, 1000),
            p_id,
            1,
            p_name,
            1,
...
begin
  g_service_name := 'Исполнение КД (распределение) на погашение ц/б';
  ...
  create_service(p_id         => g_service_id,
                 ...
                 p_start      => to_date('01.01.0001 09:00:00', 'dd.mm.yyyy hh24:mi:ss'),
                 p_end        => to_date('01.01.0001 23:59:59', 'dd.mm.yyyy hh24:mi:ss'),
                 p_periodtype => 2, --минуты
                 p_period     => 30, --раз в 30 минут
                 p_macrofile  => 'SecurRedemptionProcessor.mac',
                 p_macrofunc  => 'PushPendingSecurRedemptions');

  g_service_name := 'Исполнение КД (погашения) на погашение ц/б';
  g_service_id := get_service_id(p_name => g_service_name);
  ...
  create_service(p_id         => g_service_id,
                 ...
                 p_end        => to_date('01.01.0001 23:59:59', 'dd.mm.yyyy hh24:mi:ss'),
                 p_periodtype => 3, --часы
                 p_period     => 2, --раз в 2 часа
                 p_macrofile  => 'SecurRedemptionProcessor.mac',
                 p_macrofunc  => 'PushPendingSecurRedemptionLinks');
end;
/