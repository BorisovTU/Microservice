                     ...
                     THEN 
                        NVL ( 
                           (SELECT MAX (op.t_OperDate) 
                              FROM dnptxop_DBT op 
                             WHERE op.t_DocKind = 4605 AND op.t_Client = q.t_CLient 
                                   AND op.t_OperDate >= 
                                          TO_DATE ('01.01.' || q.ObjYear, 'DD.MM.YYYY')), 
                           TO_DATE ('01.01.0001', 'DD.MM.YYYY')) 
                     ELSE 
                        TO_DATE ('31.12.' || q.ObjYear, 'DD.MM.YYYY') 
                  END) 
                    AS LastOpDate 
            FROM (SELECT obj.t_Client, EXTRACT(YEAR FROM obj.t_Date) AS ObjYear
                    FROM dnptxobj_dbt obj 
                   WHERE obj.t_AnaliticKind2 = 2010 
                     AND NOT EXISTS(SELECT 1 FROM dnptxlnk_dbt lnk WHERE lnk.t_ID = obj.t_Analitic2) 
                   GROUP BY obj.t_Client, EXTRACT(YEAR FROM obj.t_Date) 
                   ORDER BY obj.t_Client ASC, EXTRACT(YEAR FROM obj.t_Date) ASC
                 ) q
         )
   LOOP

     v_BegDate := TO_DATE ('01.01.'||cClientData.ObjYear, 'DD.MM.YYYY');

     IF cClientData.ObjYear = 2024 THEN
       v_EndDate := cClientData.LastOpDate;
       v_SubKind := 20; --Обычный расчет
     ELSE
       v_EndDate := TO_DATE ('31.12.'||cClientData.ObjYear, 'DD.MM.YYYY');
       v_SubKind := 10; --Окончание года 
     END IF;
     
     IF v_EndDate <> TO_DATE ('01.01.0001', 'DD.MM.YYYY') THEN
       insertOP (v_BegDate,
                 v_EndDate,
                 v_EndDate,
                 v_SubKind,
