...
BEGIN
   ...
   FOR cClientData
      IN (SELECT DISTINCT s.t_PartyID
            FROM ddlcontr_dbt dlc
                 INNER JOIN ddlcontrmp_dbt mp ON mp.t_DlContrID = dlc.t_DlContrID
                 INNER JOIN dsfcontr_dbt s ON s.t_ID = mp.t_SfContrID and s.t_ServKind = 1
                 INNER JOIN dparty_dbt p ON p.t_PartyID = s.t_PartyID and p.t_LegalForm = 2
                  INNER JOIN ddl_tick_dbt t ON t.t_BOfficeKind = 101 and t.t_ClientContrID = mp.t_SfContrID AND t.t_dealdate <= TO_DATE ('28122023', 'ddmmyyyy') AND t.t_dealdate > TO_DATE ('31122022', 'ddmmyyyy')
           WHERE dlc.t_IIS <> CHR(88)
             AND NOT EXISTS (SELECT /*+ index(lot dnptxlot_dbt_idx1)*/ 1
                               FROM dnptxlot_dbt lot
                              WHERE lot.t_DocKind = t.t_BOfficeKind
                                AND lot.t_DocID = t.t_DealID
                                AND lot.t_fiid = t.t_pfi
                            )
         )
   LOOP 
     v_BegDate := TO_DATE ('01.01.2023', 'DD.MM.YYYY');
     v_EndDate := TO_DATE ('31.12.2023', 'DD.MM.YYYY');
     v_SubKind := 10; --Окончание года 
     insertOP (v_BegDate,
               v_EndDate,
               v_EndDate,
               v_SubKind,
               cClientData.t_PartyID,
               CNST.UNSET_CHAR,
               '63016');
   END LOOP; 
   --Вставки
