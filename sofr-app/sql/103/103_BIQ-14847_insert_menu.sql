...
BEGIN
   INSERT INTO DITEMSYST_DBT(T_CIDENTPROGRAM,T_ICASEITEM,T_IKINDMETHOD,T_IKINDPROGRAM,T_IHELP,T_RESERVE,T_SZNAMEITEM,T_PARM )
               VALUES('S',20302,1,2,0,CHR(1),'Еженедельный отчет по сделкам РЕПО','');
               
END;
/


/*Добавить родительский пункт Отчеты для уполномоченных представителей БР*/
DECLARE
   v_cnt                 NUMBER := 0;
   v_NumberPoint         NUMBER := 30;
   ...
   BEGIN
      ...
      IF (v_cnt = 0) AND (V_NUMBERFATHER != -1)
      THEN
         SELECT NVL (MAX (t_inumberpoint), 0)
           INTO v_NumberPoint
           FROM dmenuitem_dbt
...
DECLARE
   ...
   C_IDENTPROGRAM_CODE   NUMBER := 83; --Подсистема
   C_CASEITEM            NUMBER := 20302;  --Номер модуля
   C_NAME                VARCHAR2 (255) := 'Еженедельный отчет по сделкам РЕПО';
   V_NUMBERFATHER        NUMBER := 0;
